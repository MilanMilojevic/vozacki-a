...
(function () {
  ...
  // broj uz „poen/poena" (revizija v150: „Falilo je 1 poena") — svaki tekst sa brojem poena ide kroz ovo
  const poenTxt = (n) => n + ' ' + L(one(Math.abs(n)) ? 'pointsOne' : 'points');
  ...
  const pocetakDanaZa = (zaDana, od) => { const d = od ? new Date(od) : new Date(); d.setHours(0, 0, 0, 0); d.setDate(d.getDate() + zaDana); return d.getTime(); };
  ...
  const STR = {
    ...
    simSub: { l: '41 pitanje · 45 min · prag 84 od 98 poena', c: '41 питање · 45 мин · праг 84 од 98 поена' },
    ...
    simFalilo: { l: 'Falilo je @1 do praga (@2). Pogrešna i neodgovorena pitanja su dole — pogrešna su već u ponavljanju.', c: 'Фалило је @1 до прага (@2). Погрешна и неодговорена питања су доле — погрешна су већ у понављању.' },
    simVezbajGreske: { l: '▶ Vežbaj ova pitanja (@1)', c: '▶ Вежбај ова питања (@1)' },
    ...
    tacnoJe: { l: 'Tačno je:', c: 'Тачно је:' },
    korakUtvrdjeno: { l: 'Utvrđeno — izlazi iz ponavljanja.', c: 'Утврђено — излази из понављања.' },
    korakIzPrve: { l: 'Tačno iz prve — još jedna provera @1, pa je utvrđeno.', c: 'Тачно из прве — још једна провера @1, па је утврђено.' },
    korakRed: { l: 'Ponavljanje: @1 od 3 tačna zaredom — sledeće @2.', c: 'Понављање: @1 од 3 тачна заредом — следеће @2.' },
    korakOdmah: { l: 'odmah', c: 'одмах' },
    korakDanas: { l: 'danas', c: 'данас' },
    korakSutra: { l: 'sutra', c: 'сутра' },
    korakZaDana: { l: 'za @1 dana', c: 'за @1 дана' },
    ...
    ponNaRedu: { l: 'na redu', c: 'на реду' },
    planUzima: { l: 'u današnjem planu: @1', c: 'у данашњем плану: @1' },
    ...
    drillEmptyBezGresaka: { l: 'Nema pitanja za ponavljanje — sve što si radio je potvrđeno. 💪', c: 'Нема питања за понављање — све што си радио је потврђено. 💪' },
    ...
    ocekIznad: { l: 'očekivano iznad praga', c: 'очекивано изнад прага' },
    ocekTesno: { l: 'iznad praga, ali tesno', c: 'изнад прага, али тесно' },
    ocekIspod: { l: 'očekivano ispod praga', c: 'очекивано испод прага' },
    ...
    porSimIsteklPrazan: { l: 'Vreme je isteklo, a nijedno pitanje nije odgovoreno — taj ispit se ne računa.', c: 'Време је истекло, а ниједно питање није одговорено — тај испит се не рачуна.' },
    porSimPrazan: { l: 'Zatečen je napušten ispit bez ijednog odgovora — obrisan je, ne računa se.', c: 'Затечен је напуштен испит без иједног одговора — обрисан је, не рачуна се.' },
    porSimIsteklo: { l: 'Vreme za ispit je isteklo dok si bio odsutan — evo rezultata. Upisan je pod vremenom kad je isteklo (@1).', c: 'Време за испит је истекло док си био одсутан — ево резултата. Уписан је под временом кад је истекло (@1).' },
    porSimDrugiProzor: { l: 'Ovaj ispit je završen u drugom prozoru — tamo je i upisan. Ovde je učitan napredak iz tog prozora.', c: 'Овај испит је завршен у другом прозору — тамо је и уписан. Овде је учитан напредак из тог прозора.' },
    drillEmptyNovi: { l: 'Ovde se skupljaju pitanja za ponavljanje: pogrešna se vraćaju odmah, pa sutra, pa za tri dana, a i pogođena iz prve dođu još jednom, za tri dana. Još nisi odgovorio nijedno pitanje.', c: 'Овде се скупљају питања за понављање: погрешна се враћају одмах, па сутра, па за три дана, а и погођена из прве дођу још једном, за три дана. Још ниси одговорио ниједно питање.' },
    ...
    tour1: { l: 'Tvoj napredak u brojkama: koliko si odgovorio; ovde će se pojaviti i koliko je za ponavljanje (danas i kasnije) i koliko si obeležio.', c: 'Твој напредак у бројкама: колико си одговорио; овде ће се појавити и колико је за понављање (данас и касније) и колико си обележио.' },
    tourPlan: { l: 'Dnevni plan: upiši kad polažeš, pa ti svaki dan kaže koliko pitanja i koja — ona koja najviše vrede na ispitu.', c: 'Дневни план: упиши кад полажеш, па ти сваки дан каже колико питања и која — она која највише вреде на испиту.' },
    ...
    tour4: { l: 'Simulacija je verna kopija pravog ispita: 41 pitanje, 45 minuta, prag 84 od 98 poena. Rana simulacija je samo merenje, ne presuda — prava provera spremnosti su simulacije pred kraj pripreme.', c: 'Симулација је верна копија правог испита: 41 питање, 45 минута, праг 84 од 98 поена. Рана симулација је само мерење, не пресуда — права провера спремности су симулације пред крај припреме.' },
    ...
    tour7: { l: 'Podešavanja se otvaraju na klik. Napredak se čuva samo u ovom pregledaču — sačuvaj ga u fajl (tu je i stalno čuvanje, gde pregledač to podržava). Uradi to odmah, za svaki slučaj.', c: 'Подешавања се отварају на клик. Напредак се чува само у овом прегледачу — сачувај га у фајл (ту је и стално чување, где прегледач то подржава). Уради то одмах, за сваки случај.' },
    ...
    iosHintNapredak: { l: '<b>Pazi:</b> ikona ima svoj, odvojen napredak — pre toga sačuvaj napredak u fajl (Podešavanja), pa ga u ikoni učitaj.', c: '<b>Пази:</b> икона има свој, одвојен напредак — пре тога сачувај напредак у фајл (Подешавања), па га у икони учитај.' },
    ...
    examDateLabel: { l: 'Datum ispita (potreban za dnevni plan i odbrojavanje):', c: 'Датум испита (потребан за дневни план и одбројавање):' },
    ...
    endKasnije: { l: 'Pogrešna iz ovog spiska koja si već ponovio (@1) vraćaju se sama kad im dođe rok — u „Ponavljanju". Ponovo ih vežbaš i pre roka, ali to ne pomera raspored.', c: 'Погрешна из овог списка која си већ поновио (@1) враћају се сама кад им дође рок — у „Понављању". Поново их вежбаш и пре рока, али то не помера распоред.' },
    ...
    naIspituNula: { l: 'na ispitu: 0 (po dosadašnjim proverama se ne izvlači)', c: 'на испиту: 0 (по досадашњим проверама се не извлачи)' },
    ...
    greskeJednom: { l: '@1 @2 si pogrešio bar jednom (nijedno još dvaput).', c: '@1 @2 си погрешио бар једном (ниједно још двапут).' },
    naKojem: { l: 'na kojem', c: 'на којем' },
    naKojima: { l: 'na kojima', c: 'на којима' },
    ...
    statsTip: { l: 'Uz svaku oblast: procenat pitanja koja si POSLEDNJI PUT odgovorio tačno (od onih koja si radio) — greška koju si posle ispravio više ne vuče broj naniže. Traka: tamno plavo — tačno poslednji put, svetlo plavo — rađeno pa pogrešeno, sivo — nije otvoreno. Dodir na naziv otvara spisak pitanja, strelica otklapa podoblasti. Boja procenta: zeleno od 85%, žuto 70–84%, crveno ispod 70%.', c: 'Уз сваку област: проценат питања која си ПОСЛЕДЊИ ПУТ одговорио тачно (од оних која си радио) — грешка коју си после исправио више не вуче број наниже. Трака: тамно плаво — тачно последњи пут, светло плаво — рађено па погрешено, сиво — није отворено. Додир на назив отвара списак питања, стрелица отклапа подобласти. Боја процента: зелено од 85%, жуто 70–84%, црвено испод 70%.' },
    ...
    planNaslovAuto: { l: 'Dnevni plan', c: 'Дневни план' },
    planTacnost: { l: 'Tačnost danas', c: 'Тачност данас' },
    podNaslov: { l: 'Moj tempo je najmanje ovo', c: 'Мој темпо је најмање ово' },
    podOpis: { l: 'Kad uključiš „Moj tempo je najmanje ovo": zbir tvojih brojeva ispod (nova + ponavljanja) je DONJI dnevni obim — plan sme da traži više kad vidi da stižeš, ali nikad manje.', c: 'Кад укључиш „Мој темпо је најмање ово": збир твојих бројева испод (нова + понављања) је ДОЊИ дневни обим — план сме да тражи више кад види да стижеш, али никад мање.' },
    ...
    podIskljucen: { l: 'Donja granica isključena — plan opet traži koliko obično stižeš.', c: 'Доња граница искључена — план опет тражи колико обично стижеш.' },
    autoPoljaPod: { l: 'Dok je izabrano „Po mom tempu", zbir ovih brojeva važi kao donji dnevni obim (prekidač „Moj tempo je najmanje ovo"). Plan sme da traži više, nikad manje.', c: 'Док је изабрано „По мом темпу", збир ових бројева важи као доњи дневни обим (прекидач „Мој темпо је најмање ово"). План сме да тражи више, никад мање.' },
    autoPoljaZakljucana: { l: 'Dok je izabrano „Po mom tempu", ovi brojevi se ne koriste — plan traži koliko obično stižeš. Izaberi „Moji brojevi" ako hoćeš svoj broj.', c: 'Док је изабрано „По мом темпу", ови бројеви се не користе — план тражи колико обично стижеш. Изабери „Моји бројеви" ако хоћеш свој број.' },
    ...
    daniUkupno: { l: 'Ukupno u poslednjih @1 sa vežbom: @2 pitanja, @3 tačnih (@4%)', c: 'Укупно у последњих @1 са вежбом: @2 питања, @3 тачних (@4%)' },
    daniUkupnoPk: { l: 'Ukupno u poslednja @1 sa vežbom: @2 pitanja, @3 tačnih (@4%)', c: 'Укупно у последња @1 са вежбом: @2 питања, @3 тачних (@4%)' },
    daniKoloneNap: { l: 'Nova i ponavljanja ne moraju da daju zbir: simulacija i vežbanje pre roka ulaze samo u tačnost.', c: 'Нова и понављања не морају да дају збир: симулација и вежбање пре рока улазе само у тачност.' },
    ...
    prioIskljucen: { l: 'Prioritet po težini na ispitu je isključen.', c: 'Приоритет по тежини на испиту је искључен.' },
    ...
    planPodesiAuto: { l: 'Podesi plan', c: 'Подеси план' },
    ...
    autoOpis: { l: 'Plan meri koliko stvarno stižeš (tvoji poslednji dani, zajedno sa simulacijom) i to vreme troši na pitanja koja najviše vrede na ispitu — ponavljanja i nova po istom merilu. Nema roka za novo gradivo, pa nema ni dana kad treba sve preći ponovo. Presuda kaže šta ovim tempom možeš da očekuješ na dan ispita.', c: 'План мери колико стварно стижеш (твоји последњи дани, заједно са симулацијом) и то време троши на питања која највише вреде на испиту — понављања и нова по истом мерилу. Нема рока за ново градиво, па нема ни дана кад треба све прећи поново. Пресуда каже шта овим темпом можеш да очекујеш на дан испита.' },
    planIspitDanas: { l: 'Ispit je danas — dnevni cilj se više ne računa.', c: 'Испит је данас — дневни циљ се више не рачуна.' },
    podBezAuto: { l: '⚠ „Moj tempo je najmanje ovo" radi samo uz „Po mom tempu" — bez toga tvoji brojevi ionako važe kao ceo cilj.', c: '⚠ „Мој темпо је најмање ово" ради само уз „По мом темпу" — без тога твоји бројеви ионако важе као цео циљ.' },
    autoUkljucen: { l: 'Plan sada ide po tvom tempu do ispita — upisani brojevi se ne koriste.', c: 'План сада иде по твом темпу до испита — уписани бројеви се не користе.' },
    autoUkljucenBezDatuma: { l: 'Uključeno — još upiši datum ispita, pa plan kreće.', c: 'Укључено — још упиши датум испита, па план креће.' },
    rucnoUpisi: { l: 'Upiši koliko novih i koliko ponavljanja dnevno hoćeš, pa „Sačuvaj cilj".', c: 'Упиши колико нових и колико понављања дневно хоћеш, па „Сачувај циљ".' },
    planAutoBezDatuma: { l: '⚠ Plan je uključen, ali bez datuma ispita ne može da računa — upiši datum iznad.', c: '⚠ План је укључен, али без датума испита не може да рачуна — упиши датум изнад.' },
    planNacinPitanje: { l: 'Kako da se računa dnevni cilj?', c: 'Како да се рачуна дневни циљ?' },
    planNacinAuto: { l: 'Po mom tempu (preporučeno)', c: 'По мом темпу (препоручено)' },
    planNacinRucno: { l: 'Moji brojevi', c: 'Моји бројеви' },
    planNacinNista: { l: 'Dnevni cilj je isključen — učiš kako hoćeš. Izaberi jedan od dva načina iznad da ga uključiš.', c: 'Дневни циљ је искључен — учиш како хоћеш. Изабери један од два начина изнад да га укључиш.' },
    planPozivNaslov: { l: 'Dnevni plan', c: 'Дневни план' },
    planPozivOpis: { l: 'Upiši kad polažeš: plan meri koliko pitanja dnevno stižeš i svaki dan ti da listu onih koja najviše vrede na ispitu.', c: 'Упиши кад полажеш: план мери колико питања дневно стижеш и сваки дан ти да листу оних која највише вреде на испиту.' },
    planPozivDatum: { l: 'Kad polažeš?', c: 'Кад полажеш?' },
    planPozivDugme: { l: '▶ Uključi dnevni plan', c: '▶ Укључи дневни план' },
    planPozivSacuvaj: { l: 'Sačuvaj datum', c: 'Сачувај датум' },
    planPozivNe: { l: 'Ne treba mi plan', c: 'Не треба ми план' },
    planPozivUkljucen: { l: 'Dnevni plan je uključen.', c: 'Дневни план је укључен.' },
    planPozivSkriven: { l: 'Plan možeš da uključiš kad hoćeš: Podešavanja → Dnevni plan.', c: 'План можеш да укључиш кад хоћеш: Подешавања → Дневни план.' },
    planPozivBezDatuma: { l: 'Prvo upiši datum ispita.', c: 'Прво упиши датум испита.' },
    autoUkljucenPod: { l: 'Plan sada ide po tvom tempu do ispita, a tvoji brojevi važe kao donja granica.', c: 'План сада иде по твом темпу до испита, а твоји бројеви важе као доња граница.' },
    autoBezDatuma: { l: '⚠ Upiši datum ispita — bez njega plan ne može da računa.', c: '⚠ Упиши датум испита — без њега план не може да рачуна.' },
    prosaoDatum: { l: '⚠ Upisani datum ispita (@1) je prošao. Ako polažeš ponovo, upiši novi datum — dnevni cilj i odbrojavanje rade samo uz datum. Ako si položio, obriši ga.', c: '⚠ Уписани датум испита (@1) је прошао. Ако полажеш поново, упиши нови датум — дневни циљ и одбројавање раде само уз датум. Ако си положио, обриши га.' },
    prioNaslov: { l: 'Prioritet po težini na ispitu', c: 'Приоритет по тежини на испиту' },
    sansaNaslov: { l: 'Šansa da položiš danas', c: 'Шанса да положиш данас' },
    sansaOd10: { l: 'Kao da danas polažeš 10 puta: položio bi otprilike @1 od 10.', c: 'Као да данас полажеш 10 пута: положио би отприлике @1 од 10.' },
    sansaOd10Malo: { l: 'Kao da danas polažeš 10 puta: verovatno nijednom.', c: 'Као да данас полажеш 10 пута: вероватно ниједном.' },
    sansaOd10Skoro: { l: 'Kao da danas polažeš 10 puta: skoro svaki put (9–10 od 10).', c: 'Као да данас полажеш 10 пута: скоро сваки пут (9–10 од 10).' },
    sansaNaDanIspita: { l: 'Na dan ispita, ako nastaviš ovim tempom: ≈ @1, šansa @2.', c: 'На дан испита, ако наставиш овим темпом: ≈ @1, шанса @2.' },
    visheOd99: { l: 'više od 99%', c: 'више од 99%' },
    skalaDanas: { l: 'Danas ≈ @1', c: 'Данас ≈ @1' },
    skalaIspit: { l: 'na dan ispita, ovim tempom, ≈ @1', c: 'на дан испита, овим темпом, ≈ @1' },
    skalaPrag: { l: '(prolaz od @1).', c: '(пролаз од @1).' },
    skalaPojas: { l: 'Ako radiš manje ≈ @1, ako više ≈ @2.', c: 'Ако радиш мање ≈ @1, ако више ≈ @2.' },
    skalaDanasKratko: { l: 'danas', c: 'данас' },
    skalaIspitKratko: { l: 'ispit', c: 'испит' },
    tempoOpis: { l: 'Urađeno u danima kad si vežbao: @1. Plan računa sa @2 dnevno.', c: 'Урађено у данима кад си вежбао: @1. План рачуна са @2 дневно.' },
    tempoPlan: { l: 'plan: @1', c: 'план: @1' },
    tempoNatpis: { l: 'Koliko si uradio u poslednjih nekoliko dana kad si vežbao (plavo), i sa koliko pitanja dnevno plan računa (crta).', c: 'Колико си урадио у последњих неколико дана кад си вежбао (плаво), и са колико питања дневно план рачуна (црта).' },
    simGrafOpis: { l: 'Rezultati simulacija redom: @1; prag @2.', c: 'Резултати симулација редом: @1; праг @2.' },
    simGrafNatpis: { l: 'Tvoje simulacije redom (zeleno — položeno, crveno — nije), crta je prag 84.', c: 'Твоје симулације редом (зелено — положено, црвено — није), црта је праг 84.' },
    planZasto: { l: 'Zašto baš ovoliko?', c: 'Зашто баш оволико?' },
    putanjaOpis: { l: 'Očekivani poeni ovim tempom: danas ≈ @1, na dan ispita ≈ @2 (prag @3).', c: 'Очекивани поени овим темпом: данас ≈ @1, на дан испита ≈ @2 (праг @3).' },
    putanjaLegenda: { l: 'Plava linija: ovim tempom; senka: ako radiš manje ili više; isprekidano: ako prestaneš da vežbaš (znanje bledi); zelena crta: prag.', c: 'Плава линија: овим темпом; сенка: ако радиш мање или више; испрекидано: ако престанеш да вежбаш (знање бледи); зелена црта: праг.' },
    putanjaPrva: { l: 'Ovo je prva procena — posle 2–3 dana vežbe biće pouzdanija.', c: 'Ово је прва процена — после 2–3 дана вежбе биће поузданија.' },
    putanjaDanas: { l: 'danas', c: 'данас' },
    putanjaIspit: { l: 'ispit', c: 'испит' },
    pokrOpis: { l: 'Sva pitanja: viđeno @1 · do ispita stižeš još @2 · ostaje neviđeno @3 · van ispita @4.', c: 'Сва питања: виђено @1 · до испита стижеш још @2 · остаје невиђено @3 · ван испита @4.' },
    pkLVid: { l: 'viđeno', c: 'виђено' },
    pkLStize: { l: 'stižeš do ispita', c: 'стижеш до испита' },
    pkLRetka: { l: 'ostaje neviđeno', c: 'остаје невиђено' },
    pkLVan: { l: 'van ispita', c: 'ван испита' },
    pokrNaIspitu: { l: 'Od neviđenih na ispitu u proseku dođe oko @1 od 41 pitanja.', c: 'Од невиђених на испиту у просеку дође око @1 од 41 питања.' },
    pokrNaIspituMalo: { l: 'Od neviđenih na ispitu u proseku dođe manje od jednog pitanja.', c: 'Од невиђених на испиту у просеку дође мање од једног питања.' },
    stoAkoDugme: { l: 'Šta ako radim više ili manje?', c: 'Шта ако радим више или мање?' },
    stoAkoRed: { l: '@1 dnevno', c: '@1 дневно' },
    stoAkoTvoj: { l: 'tvoj tempo', c: 'твој темпо' },
    stoAkoPotreban: { l: 'dovoljno za 70%', c: 'довољно за 70%' },
    stoAkoNema: { l: 'Za ovo još nema dovoljno podataka.', c: 'За ово још нема довољно података.' },
    stoAkoObjasnjenje: { l: 'Šansa na dan ispita ako od danas radiš toliko pitanja dnevno (u danima kad vežbaš). Preko nekog broja dobitak je sve manji.', c: 'Шанса на дан испита ако од данас радиш толико питања дневно (у данима кад вежбаш). Преко неког броја добитак је све мањи.' },
    sansaKakoDugme: { l: 'Kako se računa šansa?', c: 'Како се рачуна шанса?' },
    sansaKako: { l: 'Za svako mesto na testu (41 pitanje, @1 poena, prag @2) računa se koliko je verovatno da ga pogodiš danas, pa tačna raspodela zbira. Gleda kako si odgovarao na svako pitanje i koliko ti pomaže kad pogrešiš pa pročitaš objašnjenje; pitanja koja još nisi video dobijaju otprilike tvoj ukupni procenat tačnih iz prve. Simulacije služe kao provera — skorašnje više. Procena, ne obećanje: kad ne vežbaš, polako pada (brzina zaborava je pretpostavka, ne meri se na tebi), a i položena simulacija sa manje poena nego što je račun očekivao ume malo da je spusti.', c: 'За свако место на тесту (41 питање, @1 поена, праг @2) рачуна се колико је вероватно да га погодиш данас, па тачна расподела збира. Гледа како си одговарао на свако питање и колико ти помаже кад погрешиш па прочиташ објашњење; питања која још ниси видео добијају отприлике твој укупни проценат тачних из прве. Симулације служе као провера — скорашње више. Процена, не обећање: кад не вежбаш, полако пада (брзина заборава је претпоставка, не мери се на теби), а и положена симулација са мање поена него што је рачун очекивао уме мало да је спусти.' },
    ...
    spremanNiz: { l: 'Poslednje @1 položene, svaka sa bar @2 preko praga', c: 'Последње @1 положене, свака са бар @2 преко прага' },
    spremanSansa: { l: 'Šansa danas bar @1% (sada @2%)', c: 'Шанса данас бар @1% (сада @2%)' },
    ...
    simUcinakVan: { l: ' (ne računa se: @1 — prazne, prekinute ili sa više od 5 neodgovorenih)', c: ' (не рачуна се: @1 — празне, прекинуте или са више од 5 неодговорених)' },
    simNijednaPrava: { l: 'Nijedna simulacija još ne ulazi u procenu: prazne, prekinute i one sa više od 5 neodgovorenih pitanja se ne računaju.', c: 'Ниједна симулација још не улази у процену: празне, прекинуте и оне са више од 5 неодговорених питања се не рачунају.' },
    sansaDanasKratko: { l: 'šansa da položiš danas', c: 'шанса да положиш данас' },
    prioOpis: { l: 'Nova pitanja idu redom od podoblasti koje ispit najviše nosi (preticanje 5 pitanja, brzine 3…), pa ono što se izostavi bude ono što se retko i pojavi. Važi za „Moji brojevi" — uz „Po mom tempu" plan ionako sve bira po vrednosti na ispitu.', c: 'Нова питања иду редом од подобласти које испит највише носи (претицање 5 питања, брзине 3…), па оно што се изостави буде оно што се ретко и појави. Важи за „Моји бројеви" — уз „По мом темпу" план ионако све бира по вредности на испиту.' },
    ...
    installWhatBody: {
      l: `<ul>
        ...
        <p><b>Šta se NE menja:</b> to je i dalje isti sajt, sa istim pitanjima. Ništa se ne preuzima iz prodavnice i ništa se ne upisuje u sistem.</p>
        <p><b>Napredak:</b> na Androidu i računaru ikona koristi isti napredak kao pregledač. <b>Na iPhone-u ikona ima SVOJ napredak</b>, odvojen od pregledača Safari: pre dodavanja sačuvaj napredak u fajl, pa ga u ikoni učitaj. Brisanjem ikone na iPhone-u briše se i napredak u njoj.</p>
      ...
      c: `<ul>
        ...
        <p><b>Шта се НЕ мења:</b> то је и даље исти сајт, са истим питањима. Ништа се не преузима из продавнице и ништа се не уписује у систем.</p>
        <p><b>Напредак:</b> на Андроиду и рачунару икона користи исти напредак као прегледач. <b>На iPhone-у икона има СВОЈ напредак</b>, одвојен од прегледача Safari: пре додавања сачувај напредак у фајл, па га у икони учитај. Брисањем иконе на iPhone-у брише се и напредак у њој.</p>
    ...
    planObjasnjenje: { l: 'Ostavi oba polja prazna ako ne želiš cilj. Kad ga postaviš, dugme na početnoj daje tačno toliko pitanja — prvo ponavljanja, pa nova. U ponavljanja ulaze i pogrešna pitanja i ona koja si pogodio iz prve (ta dobiju jednu potvrdu posle 3 dana). Ako pitanja na redu nema dovoljno, kvota se dopunjava utvrđenim pitanjima koja nisi video duže od 21 dan. Kvotu pune samo odgovori na pitanja koja su bila na redu i te dopune — vežbanje pre roka i prelistavanje utvrđenih je ne pune, ma kakav bio ishod. Ovo važi za „Moji brojevi". Uz „Po mom tempu" plan traži koliko obično stižeš (meri se iz tvojih poslednjih dana) i ponavljanja i nova bira po istom merilu — koliko vrede na ispitu.', c: 'Остави оба поља празна ако не желиш циљ. Кад га поставиш, дугме на почетној даје тачно толико питања — прво понављања, па нова. У понављања улазе и погрешна питања и она која си погодио из прве (та добију једну потврду после 3 дана). Ако питања на реду нема довољно, квота се допуњава утврђеним питањима која ниси видео дуже од 21 дан. Квоту пуне само одговори на питања која су била на реду и те допуне — вежбање пре рока и прелиставање утврђених је не пуне, ма какав био исход. Ово важи за „Моји бројеви". Уз „По мом темпу" план тражи колико обично стижеш (мери се из твојих последњих дана) и понављања и нова бира по истом мерилу — колико вреде на испиту.' },
    novihLbl: { l: 'Nova pitanja', c: 'Нова питања' },
    ponLbl: { l: 'Ponavljanja', c: 'Понављања' },
    planOstaje: { l: 'ostaje', c: 'остаје' },
    planDanasUk: { l: 'Danas urađeno', c: 'Данас урађено' },
    planDeloviTxt: { l: 'urađeno @1 · lista @2', c: 'урађено @1 · листа @2' },
    planDeloviSim: { l: ' · simulacija @1', c: ' · симулација @1' },
    planAutoGotovo: { l: '✅ Plan za danas je urađen.', c: '✅ План за данас је урађен.' },
    planJos: { l: '▶ Još 20 najvrednijih', c: '▶ Још 20 највреднијих' },
    planJosKratko: { l: 'još 20', c: 'још 20' },
    planNijeIzracunat: { l: 'Plan trenutno ne može da se izračuna — osveži stranu.', c: 'План тренутно не може да се израчуна — освежи страну.' },
    ...
    tblJos: { l: 'prevuci tabelu u stranu da vidiš ostale kolone', c: 'превуци табелу у страну да видиш остале колоне' },
    blizSlikaOpis: { l: 'slika skoro istog pitanja', c: 'слика скоро истог питања' },
    ...
    <p class="mut">Казнене мере учи последње и без учења износа напамет — у званичном испиту за А категорију те области нема.</p>` },
    ...
    posTacnoLbl: { l: 'poslednji put tačno', c: 'последњи пут тачно' },
    maloPodataka: { l: 'malo odgovora — procenat posle bar 5', c: 'мало одговора — проценат после бар 5' },
    crticaLegenda: { l: '„—" umesto procenta: manje od 5 odgovora u toj oblasti — procenat bi bio slučajnost.', c: '„—" уместо процента: мање од 5 одговора у тој области — проценат би био случајност.' },
    ...
    persistNote: { l: 'Napredak se čuva u ovom pregledaču i preživi restart, ali ne i „brisanje podataka pregledanja" ni privatni prozor. Za svaki slučaj poveži fajl za automatski upis.', c: 'Напредак се чува у овом прегледачу и преживи рестарт, али не и „брисање података прегледања" ни приватни прозор. За сваки случај повежи фајл за аутоматски упис.' },
    persistNoteBezFSA: { l: 'Napredak se čuva u ovom pregledaču i preživi restart, ali ne i „brisanje podataka pregledanja" ni privatni prozor. Za svaki slučaj povremeno sačuvaj napredak u fajl (dugme ispod).', c: 'Напредак се чува у овом прегледачу и преживи рестарт, али не и „брисање података прегледања" ни приватни прозор. За сваки случај повремено сачувај напредак у фајл (дугме испод).' },
    persistNoteIOS: { l: 'Na iPhone-u Safari sam briše podatke sajta koji ne otvoriš tokom 7 dana korišćenja Safarija, a ikona na početnom ekranu ima svoj napredak, odvojen od Safarija. Zato povremeno sačuvaj napredak u fajl (dugme ispod).', c: 'На iPhone-у Safari сам брише податке сајта који не отвориш током 7 дана коришћења Safari-ја, а икона на почетном екрану има свој напредак, одвојен од Safari-ја. Зато повремено сачувај напредак у фајл (дугме испод).' },
    ...
    backupNA: { l: '(automatski upis u fajl nije podržan u ovom pregledaču — koristi dugme za ručno čuvanje)', c: '(аутоматски упис у фајл није подржан у овом прегледачу — користи дугме за ручно чување)' },
    ...
    // broj uz dugme je inQueue (red za ponavljanje) — u njemu su i pitanja pogođena iz prve
    // koja čekaju potvrdu, pa „Samo pogrešna" nije bila istina
    onlyWrong: { l: 'Za ponavljanje', c: 'За понављање' },
    ...
    readyTitle: { l: '🎯 Spremnost danas (procena)', c: '🎯 Спремност данас (процена)' },
    readyNote: { l: 'Očekivani gubitak poena po mestima na testu, iz iste procene (41 pitanje, 98 poena, prag @1).', c: 'Очекивани губитак поена по местима на тесту, из исте процене (41 питање, 98 поена, праг @1).' },
    readyZastarelo: { l: '⚠ Poslednja simulacija je starija od 7 dana (ili je nema) — zaborav se tada samo pretpostavlja. Jedna simulacija čini procenu tačnijom.', c: '⚠ Последња симулација је старија од 7 дана (или је нема) — заборав се тада само претпоставља. Једна симулација чини процену тачнијом.' },
    readyLoss: { l: 'Najviše te košta', c: 'Највише те кошта' },
    mapaNaslov: { l: 'Mapa ispita', c: 'Мапа испита' },
    mapaOpis: { l: 'Svako polje je jedno mesto na ispitu (41), po oblastima; broj u polju su poeni, boja — koliko je verovatno da ga pogodiš danas. Dodir otvara to gradivo.', c: 'Свако поље је једно место на испиту (41), по областима; број у пољу су поени, боја — колико је вероватно да га погодиш данас. Додир отвара то градиво.' },
    mapaL4: { l: 'skoro sigurno', c: 'скоро сигурно' },
    mapaL3: { l: 'dobro', c: 'добро' },
    mapaL2: { l: 'nesigurno', c: 'несигурно' },
    mapaL1: { l: 'slabo', c: 'слабо' },
    pojNaIspitu: { l: 'na ispitu ≈ @1 pitanja', c: 'на испиту ≈ @1 питања' },
    pojRetko: { l: 'na ispitu retko ili nikad', c: 'на испиту ретко или никад' },
    rezervaIznad: { l: 'Očekuješ oko @1 više od praga (@2) — to je tvoja rezerva.', c: 'Очекујеш око @1 више од прага (@2) — то је твоја резерва.' },
    rezervaIspod: { l: 'Do praga (@2) fali oko @1.', c: 'До прага (@2) фали око @1.' },
    rezervaLegenda: { l: 'Traka: zeleno — poeni koje očekuješ; crveno — pet mesta gde gubiš najviše (tabela ispod); sivo — ostali gubici; crta — prag.', c: 'Трака: зелено — поени које очекујеш; црвено — пет места где губиш највише (табела испод); сиво — остали губици; црта — праг.' },
    mapaMale: { l: 'Ostalih @1 oblasti, po jedno pitanje', c: 'Осталих @1 области, по једно питање' },
    planUzimaNula: { l: 'plan danas daje prednost novim pitanjima (više vrede)', c: 'план данас даје предност новим питањима (више вреде)' },
    ...
    brojeviTip: { l: '<b>Za ponavljanje</b> su pitanja koja još nisu utvrđena — svako od njih nosi svoj rok, datum kad treba ponovo da ga vidiš.<br>· <b>danas</b> — rok im je danas ili je već prošao — na redu su; kad je dnevni plan uključen, on bira koja prvo, po vrednosti na ispitu.<br>· <b>kasnije</b> — rok im tek dolazi, nekog od narednih dana. Svakog jutra deo njih pređe u „danas“.<br>· <b>za osvežavanje</b> — utvrđena pitanja koja nisi video duže od 21 dan; nisu u redu, ali ih plan dopunjuje kad spremnih nema dovoljno.<br><br><b>Zašto isti broj ume da stoji na dva mesta:</b> „Danas“ broji sva pitanja koja si danas radio, svako najviše jednom. „Ponavljanja“ u dnevnom cilju broje samo pitanja koja su bila NA REDU (rok im je stigao) i provere za osvežavanje — ne nova, ne pre roka, ne prelistavanje utvrđenih. Zato ta dva broja umeju da budu ista, a umeju i da se razlikuju.<br><br><b>Zašto red raste dok učiš:</b> svako novo pitanje ulazi u red i traži bar jednu potvrdu. Dok god otvaraš nova, red raste; počinje da pada kad prestaneš da dodaješ nova.', c: '<b>За понављање</b> су питања која још нису утврђена — свако од њих носи свој рок, датум кад треба поново да га видиш.<br>· <b>данас</b> — рок им је данас или је већ прошао — на реду су; кад је дневни план укључен, он бира која прво, по вредности на испиту.<br>· <b>касније</b> — рок им тек долази, неког од наредних дана. Сваког јутра део њих пређе у „данас“.<br>· <b>за освежавање</b> — утврђена питања која ниси видео дуже од 21 дан; нису у реду, али их план допуњује кад спремних нема довољно.<br><br><b>Зашто исти број уме да стоји на два места:</b> „Данас“ броји сва питања која си данас радио, свако највише једном. „Понављања“ у дневном циљу броје само питања која су била НА РЕДУ (рок им је стигао) и провере за освежавање — не нова, не пре рока, не прелиставање утврђених. Зато та два броја умеју да буду иста, а умеју и да се разликују.<br><br><b>Зашто ред расте док учиш:</b> свако ново питање улази у ред и тражи бар једну потврду. Док год отвараш нова, ред расте; почиње да пада кад престанеш да додајеш нова.' },
    ...
    readyMalo: { l: 'Šansu još ne računam: vežbaj pitanja iz bar trećine ispita (sada je pokriveno @1%) ili uradi jednu simulaciju i odgovori na sva pitanja — prazna ne ulaze u procenu, a na ispitu pogađanje ne košta. Rana simulacija je merenje, ne presuda. Broj bez dokaza bi bio izmišljen.', c: 'Шансу још не рачунам: вежбај питања из бар трећине испита (сада је покривено @1%) или уради једну симулацију и одговори на сва питања — празна не улазе у процену, а на испиту погађање не кошта. Рана симулација је мерење, не пресуда. Број без доказа би био измишљен.' },
    doPrveProcene: { l: 'Do prve procene: pokriveno @1% ispita, treba 33%', c: 'До прве процене: покривено @1% испита, треба 33%' },
  ...
  // JEDNA mera „tačnosti" za svaki spisak (statistika, strana oblasti, „Sva pitanja"): procenat
  // pitanja koja su POSLEDNJI PUT odgovorena tačno, od odgovorenih. Do v147 su zaglavlja spiskova
  // i dalje računala udeo tačnih u SVIM odgovorima ikad, pa je ista oblast na statistici imala
  // 92%, a na svojoj strani 71% i crveno.
  function tacnostSpiska(ids) {
    let seen = 0, good = 0;
    for (const id of ids) { const r = S.q[id]; if (r && r.a) { seen++; if (r.streak >= 1) good++; } }
    return { seen, good, acc: seen ? Math.round(100 * good / seen) : null };
  }
  ...
  function normalizeState(obj) {
    ...
    const sims = (Array.isArray(obj.sims) ? obj.sims : []).slice(-500).map((s) => {
      ...
      return {
        ...
        passed: !!s.passed,
        wrong: ids(s.wrong),
        // broj odgovorenih (od v145): prekinut ispit nije uzorak znanja — procena ga preskače
        ...(Number.isInteger(s.odg) ? { odg: nInt(s.odg, 0, 200, 0) } : {}),
        // koliko je ispit dodao dnevnom broju (v149): pitanje već odgovoreno tog dana se ne broji
        // dvaput, pa plan iz ovoga zna da li je dan sa simulacijom posle liste stvarno urađen
        ...(Number.isInteger(s.dn) ? { dn: nInt(s.dn, 0, 200, 0) } : {}),
    ...
    // JEDNOKRATNA ISPRAVKA (v145). Pre v139 (17.09.2026) finishSim je i NEODGOVORENO pitanje
    // upisivao u napredak kao grešku. Prazan ispit (0 poena, sva mesta u spisku grešaka, nijedan
    // izbor) ostavljao je tako 41 LAŽNU grešku. Kod Milana dva takva ispita = 82 greške: „pogrešeno
    // dvaput ili više" bilo je 59 umesto 45, a red za ponavljanje i procena su nosili nešto što se
    // nije desilo. Poništava se po jedna greška po pitanju iz takvog ispita — samo za ispite pre v139
    // (posle se neodgovoreno ne upisuje) i samo jednom (bit 1 u `isp`).
    let isp = nInt(obj.isp, 0, 1e6, 0);
    if (!(isp & 1)) {
      const V139 = Date.UTC(2026, 8, 17, 11, 10, 23);
      for (const s of sims) {
        const bezIzbora = !s.qs || s.qs.every((e) => !e.ch.length);
        if (s.score !== 0 || s.d >= V139 || !bezIzbora || s.wrong.length < 41) continue;
        for (const id of s.wrong) {
          const r = q[id];
          if (!r || r.w < 1 || r.a < 1) continue;
          r.a -= 1; r.w -= 1;
          if (r.streak > r.a - r.w) r.streak = r.a - r.w;
          if (!r.a) delete q[id];
        }
      }
      isp |= 1;
    ...
    // Ispravka se vezuje i za SAME podatke: stara kopija aplikacije (≤ v144) pri uvozu izbaci
    // polje isp, pa bi se oduzimanje primenilo DRUGI put (revizija v148). Zato se spisak
    // „grešaka" takvog praznog ispita prazni — to ionako nisu bile greške nego neodgovorena
    // pitanja — i posle toga ispravka nema šta ponovo da oduzme, ma koliko se puta pokrenula.
    for (const s of sims) {
      const bezIzbora = !s.qs || s.qs.every((e) => !e.ch.length);
      if (s.score === 0 && s.d < Date.UTC(2026, 8, 17, 11, 10, 23) && bezIzbora && s.wrong.length >= 41) s.wrong = [];
    }
    ...
    const planObj = obj.plan && typeof obj.plan === 'object' && !Array.isArray(obj.plan)
      ? {
        ...
        auto: obj.plan.auto === 1 ? 1 : 0,   // plan po tempu (plan.js): koliko stižeš, po vrednosti na ispitu
        ...
        // v149: dnevni zapis plana ({dan: {n0, k, t}}) — iz njega se sutra meri koliko stvarno stižeš.
        // Bez njega bi se procena kapaciteta posle svakog učitavanja vraćala na probu.
        ...(() => {
          const tr = obj.plan.prognoza && obj.plan.prognoza.tr;
          if (!tr || typeof tr !== 'object' || Array.isArray(tr)) return {};
          const out = {};
          for (const [d, x] of Object.entries(tr)) {
            if (!/^\d{4}-\d{2}-\d{2}$/.test(d) || !x || typeof x !== 'object') continue;
            const n0 = nInt(x.n0, 0, 1e5, null), k = nInt(x.k, 0, 1e5, null), t = nNum(x.t, 0, maxTs(), null);
            if (n0 !== null && k !== null && t !== null) {
              // v150: plan pamti i koliko je tražio (kap), da li je lista bila kraća jer vrednijeg nije bilo
              // (dosta), da li je čuvao mesto simulaciji (r), listu dana (ids) i koliko je od nje urađeno (u),
              // i da li je simulacija urađena pre liste (s). Bez ovoga se „dan stignut" gubio pri osvežavanju.
              const zz = { n0, k, t };
              const kap = nInt(x.kap, 1, 1e5, null); if (kap !== null) zz.kap = kap;
              if (x.dosta === 1) zz.dosta = 1;
              if (x.r === 0 || x.r === 1) zz.r = x.r;
              if (x.s === 0 || x.s === 1) zz.s = x.s;
              if (Array.isArray(x.ids)) zz.ids = x.ids.slice(0, 400).filter((id) => Number.isInteger(id) && id > 0 && id <= 1e6);
              const u = nInt(x.u, 0, 400, null); if (u !== null) zz.u = u;
              out[d] = zz;
            }
          }
          const kl = Object.keys(out).sort().slice(-14);
          return kl.length ? { prognoza: { tr: Object.fromEntries(kl.map((d) => [d, out[d]])) } } : {};
        })(),
      }
      : null;
    ...
    return {
      ...
      planNe: obj.planNe === 1 ? 1 : 0,   // v150: sklonio je poziv za dnevni plan sa početne
  ...
  function poruci(tekst) {
    try {
      let drz = document.getElementById('trakeDrzac');
      if (!drz) {
        drz = document.createElement('div'); drz.id = 'trakeDrzac';
        drz.setAttribute('role', 'status'); drz.setAttribute('aria-live', 'polite');
        document.body.appendChild(drz);
      }
      ...
      b.textContent = tekst;
      drz.appendChild(b);
      // revizija v149: 14 od 27 poruka je bilo duže nego što prosečan čitalac stigne za 2,6 s
      const reci = String(tekst).split(/\s+/).length;
      const traje = Math.max(4000, 2000 + 400 * reci);
      let tajmer = null;
      const kreni = () => { clearTimeout(tajmer); tajmer = setTimeout(() => b.remove(), traje); };
      b.addEventListener('mouseenter', () => clearTimeout(tajmer));
      b.addEventListener('mouseleave', kreni);
      b.addEventListener('touchstart', () => clearTimeout(tajmer), { passive: true });
  ...
  // Naziv podoblasti iz baze ume da se završi sa „; " (23 od 89 naziva) — to je ostatak
  // nabrajanja iz pravilnika, ne deo imena. Seče se na JEDNOM mestu, pa svuda izgleda isto.
  const subPar = (sid) => ({ l: String(D.subs[sid].l).replace(/[\s;,]+$/, ''), c: String(D.subs[sid].c).replace(/[\s;,]+$/, '') });
  const subIme = (sid) => T(subPar(sid));
  const subOf = (q) => subIme(q.sub);
  ...
  // kad (neobavezno): trenutak odgovora kad NIJE sada — ispit koji je istekao dok korisnika nije
  // bilo upisuje se u trenutku roka, ne povratka (revizija v148: inače su odgovori od pre tri
  // dana „osvežavali" pitanja i punili današnji dan i dnevni cilj).
  function record(id, ok, izbor, kad) {
    const r = qs(id);
    const prviPut = !r.a;             // pre uvećanja: ovo pitanje se danas radi kao NOVO
    const sada = Number.isFinite(kad) ? Math.min(kad, Date.now()) : Date.now();
    ...
    const bioURedu = inQueue(id);
    const bioRok = bioURedu && dueOf(id) <= sada;
    // osveženje: utvrđeno pitanje neviđeno duže od 21 dan — planIds ga služi kao dopunu kvote
    const osvezenje = !prviPut && !bioURedu && !!r.last && (sada - r.last > OSVEZI_POSLE);
    const preRoka = ok && !prviPut && bioURedu && !bioRok;
    ...
    const danOdg = localDay(sada);
    const vecBrojanoDanas = !!r.last && localDay(r.last) === danOdg;
    if (preRoka && !r.due) r.due = dueOf(id);   // nasleđen zapis bez roka: zamrzni rok pre pomeranja r.last
    r.a++; r.last = Math.max(r.last || 0, sada);
    ...
    if (preRoka) {
      ...
      r.streak++;
      if (r.w > 0 && r.streak < 3) r.due = pocetakDanaZa(r.streak === 1 ? 1 : 3, sada);
      else if (r.w === 0 && r.streak === 1) r.due = pocetakDanaZa(3, sada);   // utvrđivanje: druga potvrda za 3 dana
      else delete r.due;                                               // utvrđeno / izašlo iz reda
    } else {
      r.w++; r.streak = 0; r.due = sada;
    ...
    const dan = danOdg === today ? S.day : danUArhivi(danOdg);
    if (!vecBrojanoDanas) {
      dan.n++; if (ok) dan.ok++;
      if (prviPut) dan.novih = (dan.novih || 0) + 1;
      // kvotu pune SAMO prava ponavljanja: odgovor na pitanje koje je bilo NA REDU (rok stigao,
      // ma kakav ishod) i provera sa spiska za osvežavanje. Ne pune je: vežbanje pre roka
      // (ma kakav ishod) i prelistavanje utvrđenih — ranije je i ono punilo traku do 60/60 ✓
      // dok stvarni red stoji netaknut.
      else if (bioRok || osvezenje) dan.pon = (dan.pon || 0) + 1;
    }
    upisiNiz(danOdg);
    save();
    return !vecBrojanoDanas;   // da li je odgovor ušao u dnevni broj (simulacija to pamti kao dn)
  }
  // Zapis PROŠLOG dana u dnevniku (za odgovore upisane naknadno, npr. istekao ispit) — pravi se
  // ako ga nema, na svom mestu po datumu, da crtež i tabela ostanu hronološki.
  function danUArhivi(d) {
    S.dani = Array.isArray(S.dani) ? S.dani : [];
    let x = S.dani.find((y) => y.d === d);
    if (!x) {
      x = { d, n: 0, ok: 0, novih: 0, pon: 0 };
      const i = S.dani.findIndex((y) => y.d > d);
      if (i < 0) S.dani.push(x); else S.dani.splice(i, 0, x);
    }
    return x;
  }
  // Niz dana zaredom: dan se upisuje jednom; dan posle upisanog produžava niz, praznina ga prekida.
  function upisiNiz(dan) {
    if (S.streakD && S.streakD >= dan) return;
    const pre = new Date(dan + 'T00:00:00'); pre.setDate(pre.getDate() - 1);
    S.streakN = S.streakD === localDay(pre.getTime()) ? S.streakN + 1 : 1;
    S.streakD = dan;
  }
  // Korak ponavljanja za jedno pitanje, posle upisa odgovora: tačkice (koliko tačnih zaredom od 3)
  // i kada se vraća. Pitanje van reda: utvrđeno. Tačno iz prve: još jedna provera.
  function korakHtml(id) {
    const r = S.q[id];
    if (!r || !r.a) return '';
    const tacke = (n, m) => '<span class="korakTacke" aria-hidden="true">' + Array.from({ length: m }, (x, i) => (i < n ? '●' : '○')).join('') + '</span> ';
    if (!inQueue(id)) return `<div class="korak">${tacke(3, 3)}${escapeHtml(L('korakUtvrdjeno'))}</div>`;
    const due = dueOf(id), sad = Date.now();
    const dana = Math.round((pocetakDana(due) - pocetakDana(sad)) / DAY);
    const kad = due <= sad ? L('korakOdmah') : dana <= 0 ? L('korakDanas') : dana === 1 ? L('korakSutra') : L('korakZaDana').split('@1').join(dana);
    if (r.w === 0) return `<div class="korak">${tacke(1, 2)}${escapeHtml(L('korakIzPrve').split('@1').join(kad))}</div>`;
    return `<div class="korak">${tacke(r.streak, 3)}${escapeHtml(L('korakRed').split('@1').join(r.streak).split('@2').join(kad))}</div>`;
  }
  const inQueue = (id) => {
    const r = S.q[id];
    if (!r) return false;
  ...
  function browseGreske() {
    ...
    head.innerHTML = `<h3>${escapeHtml(title)}</h3>
      <div class="mut opisRed">${escapeHtml(L(dvaput ? 'greskeOpis' : 'greskeJednom').split('@1').join(nQ(ids.length)).split('@2').join(L(one(ids.length) ? 'naKojem' : 'naKojima')))}</div>
  ...
  // Datum ispita: ista provera za Podešavanja i za karticu na početnoj. Vraća tekst greške ili null.
  function greskaDatumaIspita(v) {
    if (!/^\d{4}-\d{2}-\d{2}$/.test(v)) return L('datumLos');
    ...
    if (isNaN(d.getTime()) || godina < danas.getFullYear() || godina > danas.getFullYear() + 5) {
      return L('datumOpseg').split('@1').join(danas.getFullYear()).split('@2').join(danas.getFullYear() + 5);
    }
    if (d < danas) return L('datumProslost');
    return null;
  }
  // Izbor „Moji brojevi" pre nego što je ijedan broj upisan: plan još ne postoji, a polja moraju da se vide.
  let rucnoUI = false;
  ...
  // Ista adresa bez novog koraka u istoriji (i bez hashchange): ekran se ne menja, samo
  // osvežavanje i deljenje od sada vode na pravo mesto.
  function zameniHash(h) {
    curHash = h;
    if (FILE_MODE) return;
    if (location.hash !== h) { try { history.replaceState(history.state, '', h); } catch (e) { /* bez istorije — adresa ostaje */ } }
  }
  ...
  function renderQuestion(opts) {
    ...
    let answered = false;
    const zapamti = (odgovoreno, lwPre) => {
      ...
      prikazPitanja.set(opts.recordKey, { order: shuffled.map((x) => x.id), sel: [...sel], odgovoreno: odgovoreno || (zapamceno && zapamceno.odgovoreno) || null,
        lwPre: lwPre !== undefined ? lwPre : ((zapamceno && zapamceno.lwPre) || null) });
    ...
    function finish(chosen) {
      answered = true;
      // „Prošli put": snimak PRE ovog odgovora. Pri ponovnom crtanju (ЋИР/LAT) record() je već
      // upisao ovaj odgovor, pa je čitanje iz S pokazivalo izbor od pre par sekundi — a posle
      // tačnog odgovora je prava prošla greška nestajala (revizija v148).
      const lwPre = zapamceno && zapamceno.odgovoreno ? (zapamceno.lwPre || null)
        : (S.q[q.id] && Array.isArray(S.q[q.id].lw) ? S.q[q.id].lw.slice() : null);
      zapamti(chosen.map((x) => x.id), lwPre);
      ...
      // uz netačan odgovor presuda kaže i koji je tačan: presuda ostaje na vrhu ekrana (v148), a zeleno
      // obeležen odgovor je tada iznad nje — „označen zelenim" upućivalo je na nešto što se ne vidi
      if (ok) v.textContent = L('correct');
      else v.innerHTML = escapeHtml(L('wrong')) + ' ' + escapeHtml(L('tacnoJe')) + ' '
        + q.ch.filter((x) => x.ok).map((x) => '<b>' + escapeHtml(T(x.t)) + '</b>').join(' · ');
      c.insertBefore(v, actions);
      const ex = explNode(q, { proslaGreska: lwPre });
      ...
      if (nextBtn) {
        ...
        if (f0 === document.body || f0 === c || c.contains(f0)) {
          ...
          // Pomera se taman toliko da se dugme vidi, ALI nikad preko presude: do v147 je
          // scrollIntoView gurao presudu i 🔑 ključ iznad ekrana čim je objašnjenje bilo duže od
          // ekrana (na 125% kod 58 od 59 pitanja sa ključem) — baš ono zbog čega se odgovara.
          // Kad je objašnjenje dugo, presuda ostaje na vrhu, a dugme je jedan skrol niže.
          if (!vidiSe) {
            const vrh = visinaTrake();
            let dy = r.top < vrh ? r.top - vrh - 8 : r.bottom - dno + 8;
            if (dy > 0) dy = Math.min(dy, Math.max(0, v.getBoundingClientRect().top - vrh - 8));
      ...
      { const kr = korakHtml(q.id); if (kr) v.insertAdjacentHTML('beforeend', kr); }
    }
  }
  ...
  function naIspitu(sid) {
    if (!_naIspitu) {
      _naIspitu = {};
      for (const [s, w] of Object.entries(tezinaPodoblasti())) {
        _naIspitu[s] = w >= 1 ? String(Math.round(w)) : '0–1';
      }
    }
    // podoblast van šablona: po dosadašnjim proverama se ne izvlači — i to se kaže (revizija v149)
    return _naIspitu[sid] || '0';
  }
  ...
  function krajSpiska(m) {
    // „Ponovi pogrešna" nudi samo ono što ponavljanje stvarno MENJA: pogrešna kojima je rok
    // stigao (pogrešeno u ovom prolazu je na redu odmah). Do v147 su se brojala sva pogrešna iz
    // reda, i ona za sutra — tačan odgovor pre roka ne pomera raspored, pa se posle potpuno
    // tačnog kruga vraćala ista cifra i dugme se vrtelo u krug (revizija v148).
    const sada = Date.now();
    const pogresnaSve = m.ids.filter((id) => inQueue(id) && S.q[id] && S.q[id].w > 0);
    const pogresna = pogresnaSve.filter((id) => dueOf(id) <= sada);
    const kasnije = pogresnaSve.length - pogresna.length;
    // Neodgovorena iz spiska: kraj „Učenja redom" je pisao ✅ i kod 403 neotvorena pitanja.
    const neodg = m.ids.filter((id) => !(S.q[id] && S.q[id].a));
    const dalje = sledecaSekcija(m.secKey || m.chainKey);
    const spremno = queueSplit().ready.length;
    const dugmad = [];
    let glavno = false;
    const klasa = () => { const k = glavno ? 'secondary' : 'primary'; glavno = true; return k; };
    if (pogresna.length) dugmad.push('<button class="' + klasa() + '" id="bEndWrong">' + L('endWrongBtn').replace('#', pogresna.length) + '</button>');
    if (neodg.length) dugmad.push('<button class="' + klasa() + '" id="bEndUnseen">' + L('onlyUnseen') + ' (' + neodg.length + ')</button>');
    if (dalje) dugmad.push('<button class="' + klasa() + '" id="bEndNext">' + (dalje.istaOblast ? L('endNextSub') : L('endNextCat')).replace('#', escapeHtml(dalje.istaOblast ? dalje.ime : dalje.oblast)) + ' ›</button>');
    if (m.plan && S.plan && S.plan.auto && (planAuto(20) || { ids: [] }).ids.length) dugmad.push('<button class="' + klasa() + '" id="bEndPlanJos">' + L('planJos') + '</button>');
    ...
    endScreen((neodg.length ? '' : '✅ ') + L('endTitle').replace('#', br).replace('&', recPitanje), m.origin, dugmad.join(''));
    if (kasnije) el('qCard').insertAdjacentHTML('beforeend', '<div class="mut napomena razmakG">' + escapeHtml(L('endKasnije').split('@1').join(kasnije)) + '</div>');
    ...
    const bu = el('bEndUnseen');
    if (bu) bu.addEventListener('click', () => startList(neodg, () => m.titleFn() + ' — ' + L('onlyUnseen').toLowerCase(), null, 'filter', { origin: m.origin, chainKey: m.secKey || m.chainKey }));
    const bpj = el('bEndPlanJos');
    if (bpj) bpj.addEventListener('click', planJos);
    ...
    // hash: bez adrese je osvežavanje usred te podoblasti vodilo na početnu bez reči.
    if (bn) bn.addEventListener('click', () => startList(Q.filter((q) => 's' + q.sub === dalje.key).map((q) => q.id), secTitleFn(dalje.key), null, 'section', { secKey: dalje.key, startAt: S.secPos[dalje.key] || 0, origin: () => browse(dalje.key), hash: '#/vezba/' + dalje.key }));
  }
  ...
  function startList(ids, titleFn, emptyMsgFn, kind, opts) {
    ...
    listMode = { ids, i: start, titleFn, kind, secKey: opts.secKey || null, chainKey: opts.chainKey || null, origin: opts.origin || null,
      nazadLbl: opts.nazadLbl || null, jedno: !!opts.jedno, hash: opts.hash || '#/vezba', plan: !!opts.plan };
  ...
  function stepList() {
    ...
    if (m.i >= m.ids.length) {
      ...
      if (m.kind === 'drill') {
        ...
        if (waiting.length) {
          ...
          el('qCard').insertAdjacentHTML('beforeend', `<div class="mut napomena razmakG">${L('preRokaNapomena')}</div>`);
      ...
      if (m.secKey) { S.secPos[m.secKey] = 0; save(); zameniHash('#/sek/' + m.secKey); }
      krajSpiska(m);
      return;
    }
    ...
    renderQuestion({
      ...
      onAnswered: (ok, izbor) => record(q.id, ok, izbor),
      onNext: m.jedno
        ? () => { lastRecordKey = null; startList(Q.filter((x) => x.sub === q.sub).map((x) => x.id), secTitleFn(kljuc), null, 'section', { secKey: kljuc, startAt: S.secPos[kljuc] || 0, origin: () => browse(kljuc), hash: '#/vezba/' + kljuc }); }
  ...
  function blizanciHtml(q) {
    const e = EX.byQ[q.id] || {};
    const provereni = (e.bl || []).filter((b) => byId.has(b.id));
    const vec = new Set(provereni.map((b) => b.id));
    const auto = ((EX.blizanci || {})[q.id] || []).filter((id) => !vec.has(id) && byId.has(id)).map((id) => ({ id }));
    const svi = [...provereni, ...auto].slice(0, 4);
    ...
    return svi.map((s) => {
      ...
      // dimenzije iz explanations.js (dim): slika bez njih zauzme mesto tek kad stigne i gurne dugme
      // „Sledeće" ispod ekrana; dugme za uvećanje kao kod slike pitanja (revizija v148)
      const dm = (EX.dim || {})[b.id];
      const slika = b.img ? `<button type="button" class="qImgBtn blizSlikaBtn" aria-label="${escapeHtml(L('uvecajSliku'))}: ${escapeHtml(L('blizSlikaOpis'))} #${b.id}"><img class="blizSlika" loading="lazy" src="img/${b.id}.jpg"${dm ? ` width="${dm[0]}" height="${dm[1]}"` : ''} alt="${escapeHtml(L('blizSlikaOpis'))} #${b.id}"></button>` : '';
      const razlika = s.r
  ...
  function explNode(q, opcije) {
    ...
    const qRefs = (h) => h.replace(/#(\d{4,5})\b/g, (m0, id) => (byId.has(+id) ? `<button type="button" class="qRef" data-q="${id}" title="${escapeHtml(L('otvoriPitanje'))}">#${id}</button>` : m0));
    ...
    if (Array.isArray(opcije.proslaGreska)) {
      const lw = opcije.proslaGreska.map((id) => q.ch.find((c) => c.id === id)).filter((c) => c && !c.ok);
  ...
  // Tabela šira od ekrana skroluje u svom okviru (style.css), ali bez ikakvog znaka da ima još
  // udesno (revizija v148: na 125% i do 28 tabela). Ispod takve tabele stoji kratka napomena;
  // meri se tek kad je kartica vidljiva, a posle promene veličine slova ponovo pri otvaranju.
  function oznaciSiroke(cd) {
    cd.querySelectorAll('table').forEach((t) => {
      const siroka = t.scrollWidth > t.clientWidth + 2;
      const n = t.nextElementSibling;
      const ima = !!(n && n.classList.contains('tblJos'));
      if (siroka && !ima) t.insertAdjacentHTML('afterend', `<div class="mut napomena tblJos">↔ ${escapeHtml(L('tblJos'))}</div>`);
      else if (!siroka && ima) n.remove();
    });
  }
  ...
  function simVrati() {
    ...
    return { qs, i: nInt(z.i, 0, SIM_N - 1, 0), showReport: z.r === 1, deadline: z.d, timerId: null, snimljeno: true };
  }
  ...
  function simNastavi() {
    ...
    const s = simVrati();
    if (!s) return false;
    // Istekao ispit u kome NIŠTA nije odgovoreno se odbacuje bez zapisa: to je slučajan dodir
    // po „Ispit" pa zatvoren tab, ne pokušaj — upis pada 0/98 bi zagadio istoriju, red
    // ponavljanja (41 × netačno) i pravilo spremnosti. Ispit sa BAR JEDNIM odgovorom se
    // završava i beleži, kao na pravom ispitu. Provera ide PRE setHash/show — posle njih bi
    ...
    if (!sim) { const zs = S.sims[S.sims.length - 1]; poruci(L('porSimIsteklo').split('@1').join(zs ? fmtDatum(zs.d, true) : '')); return true; }
  ...
  function finishSim(auto) {
    if (!sim) return;
    // Drugi prozor je ovaj isti ispit već završio (ili pokrenuo nov): zapis toka je obrisan ili
    // nosi drugi rok. Ovaj prozor ga tada NE sme upisati — save() bi preko napretka iz drugog
    // prozora prepisao svoje, staro stanje (revizija v148). Proverava se samo ako je zapis toka
    // ovde ikad uspeo: bez skladišta nema ni drugog prozora koji bi ga video.
    if (sim.snimljeno) {
      let zt = null;
      try { zt = JSON.parse(localStorage.getItem(SIM_KEY) || 'null'); } catch (e) { zt = null; }
      if (!zt || zt.d !== sim.deadline) {
        clearInterval(sim.timerId);
        sim = null;
        S = load();                  // napredak iz drugog prozora, ne ovaj zastareli
        poruci(L('porSimDrugiProzor'));
        setHash('#/'); renderHome(); show('home');
        return;
      }
    }
    // Istekao ispit u kome ništa nije odgovoreno se odbacuje — isto pravilo koje simNastavi
    // primenjuje pri osvežavanju. Ranije je ishod zavisio od toga da li je stranica ostala
    // otvorena: uživo se upisivalo 0/98 i 41 × netačno u red, a posle osvežavanja se brisalo.
    // Ručni „Kraj ispita" (auto = false) NAMERNO ostaje pokušaj — to je svesna odluka čoveka.
    if (auto && !sim.qs.some((sq) => sq.chosen.size > 0)) {
      clearInterval(sim.timerId);
      sim = null;                    // pre poruke i rutiranja: leaveSimOk ne sme da traži potvrdu za ispit kog nema
      simObrisi();
      poruci(L('porSimIsteklPrazan'));
      setHash('#/'); renderHome(); show('home');
      return;
    }
    if (!auto) {
    ...
    clearInterval(sim.timerId);
    // ispit koji je sam istekao završava se u trenutku ROKA, ne kad se korisnik vratio
    const kraj = auto ? Math.min(Date.now(), sim.deadline) : Date.now();
    const total = sim.qs.reduce((a, sq) => a + sq.q.pts, 0);
    let score = 0;
    const wrong = [];
    let dn = 0;   // koliko je ovaj ispit dodao dnevnom broju
    for (const sq of sim.qs) {
      ...
      if (sq.chosen.size > 0 && record(sq.q.id, ok, [...sq.chosen], kraj)) dn++;
    ...
    const rec = { d: kraj, score, total, passed, wrong: wrong.map((x) => x.q.id), odg: sim.qs.filter((sq) => sq.chosen.size > 0).length, dn, qs: sim.qs.map((sq) => ({ id: sq.q.id, ch: [...sq.chosen] })) };
    S.sims.push(rec);
  ...
  function renderSimReview(rec, fresh) {
    ...
    rc.innerHTML = `<h3>${L('sim')} <span class="mut" style="font-weight:normal">· ${fmtDatum(rec.d, true)}</span></h3>
      ...
      ${!rec.passed && (rec.wrong || []).length ? `<p class="mut napomena">${escapeHtml(L('simFalilo').split('@1').join(poenTxt(Math.max(0, prag(rec.total) - rec.score))).split('@2').join(prag(rec.total)))}</p>
        <div class="qActions"><button type="button" class="secondary" id="btnSimVezbajGreske">${escapeHtml(L('simVezbajGreske').split('@1').join((rec.wrong || []).length))}</button></div>` : ''}
    ...
    const bvg = rc.querySelector('#btnSimVezbajGreske');
    if (bvg) bvg.addEventListener('click', () => startList((rec.wrong || []).filter((id) => byId.has(id)), () => L('simWrongTitle'), null, 'filter', { origin: () => renderSimReview(rec, false) }));
  ...
  // izabrana merenjem (tools/kalibracija-sanse.mjs: veštački učenici čije se stvarno znanje zna)
  // između četiri nezavisna predloga: prosečna greška šanse 5,9 p.p. naspram 45 kod stare, i
  // 7,3 naspram 46,6 na scenarijima koje niko nije video pre ocenjivanja.
  // Isti fajl učitava i merni alat, pa se meri tačno ono što se isporučuje.
  // Račun traje desetine milisekundi, a traži se na više mesta pri svakom crtanju početne — zato
  // se pamti dok se stanje ne promeni (i najviše sat, jer zaborav zavisi od vremena).
  let _procena = null;
  function readiness() {
    let potpis = S.sims.length * 7919 + (S.sims.length ? S.sims[S.sims.length - 1].d % 1e9 : 0) + Math.floor(Date.now() / 3600e3) * 131;
    let answered = 0;
    for (const id in S.q) {
      const r = S.q[id];
      if (r.a > 0) answered++;
      potpis = (potpis * 31 + (+id) * 3 + r.a * 17 + r.w * 29 + r.streak * 41 + ((r.last || 0) % 1e6)) % 2147483647;
    }
    if (_procena && _procena.potpis === potpis) return _procena.rez;
    const P = window.VozackiProcena;
    let r = null;
    try { r = P ? P.proceni(S, { Q, byId, SIM_SLOTS, prag, sad: Date.now(), DAY }) : null; }
    catch (err) { console.warn('procena šanse:', err); r = null; }   // bez broja je bolje nego pogrešan broj
    const loss = {};   // podoblasti slota -> {pts, subs}: očekivani gubitak poena
    if (r && r.slotovi) for (const s of r.slotovi) (loss[s.subs.join('/')] = loss[s.subs.join('/')] || { pts: 0, subs: s.subs }).pts += s.pts * (1 - s.p);
    const rez = { exp: r && r.exp != null ? r.exp : null, sansa: r && r.sansa != null ? r.sansa : null, loss, answered, slotovi: r ? r.slotovi : null, pokriveno: r && Number.isFinite(r.pokriveno) ? r.pokriveno : 0 };
    _procena = { potpis, rez };
    return rez;
  }
  ...
  function spremnost() {
    ...
    return {
      ...
      // poređenje na ZAOKRUŽENOM procentu: 84,6% se prikazuje kao 85%, pa uslov ne sme da
      // kaže ✗ pored velikog zelenog „85%"
      sansaOk: !!s && Math.round(s.sansa * 100) >= Math.round(SIM_SANSA * 100),
    };
  }
  function renderReady() {
    const { exp, loss, answered, pokriveno } = readiness();
    if (exp === null) {
      // Procena vraća „nema broja" kad nema simulacije, a vežbano je premalo ispita. Jedna simulacija
      // to odmah rešava — zato i dugme za nju, ne samo „nastavi učenje".
      // umesto golog „pokriveno 19%": traka koliko je puta do prve procene (prag je trećina ispita)
      const doProcene = Math.min(100, Math.round(100 * pokriveno * 3));
      el('readyCard').innerHTML = `<h3>${L('readyTitle')}</h3>
        <div class="planRed"><span class="planIme">${L('doPrveProcene').split('@1').join('<b>' + Math.round(100 * pokriveno) + '</b>')}</span><span class="planBar"><span style="width:${doProcene}%"></span></span><span></span></div>
        <p class="mut">${L('readyMalo').split('@1').join(Math.round(100 * pokriveno))}</p>
        <div class="qActions"><button type="button" class="primary" data-nav="learn">${L('krenimo')}</button> <button type="button" class="secondary" data-nav="sim">${L('sim')}</button></div>`;
    ...
    // Procena NIJE ishod ispita — zato ne „POLOŽIO": zelen natpis je do v147 stajao iznad crvene
    // šanse (prosek iznad praga, šansa ispod 85%). Natpis sad kaže šta broj jeste, a boja prati i
    // šansu: iznad praga uz šansu ispod praga pravila je „tesno", ne zeleno.
    const tesno = pass && pct !== null && pct < Math.round(SIM_SANSA * 100);
    const ocena = !pass ? 'fail' : tesno ? 'tesno' : 'pass';
    el('readyCard').innerHTML = `<h3>${L('readyTitle')}</h3>
      <div class="bigScore ${ocena}">≈ ${e} / ${SIM_PTS_MIN}</div>
      ${(() => {
        // ista skala kao na početnoj: danas i — ako je plan uključen — dan ispita ovim tempom
        const ra = S.plan && S.plan.auto && danaDoIspita() > 0 ? planAuto() : null;
        const na = ra ? ispitNastavak(ra) : null;
        const ok = !!na && na.ocena !== 'nema' && Number.isFinite(na.E);
        const pj = ok ? pojasIspita(ra) : {};
        return skalaSvg({ danas: exp, ispit: ok ? na.E : undefined, manje: pj.manje, vise: pj.vise, ocena: ok ? na.ocena : 'nema' })
          + (ok && Number.isFinite(na.P) ? `<p class="mut napomena">${escapeHtml(L('sansaNaDanIspita').split('@1').join(poenTxt(Math.round(na.E))).split('@2').join(prPlan(na.P)))}</p>` : '');
      })()}
      <p><span class="pill ${ocena}">${L(!pass ? 'ocekIspod' : tesno ? 'ocekTesno' : 'ocekIznad')}</span>
      ...
      <div class="bigScore ${pct >= Math.round(SIM_SANSA * 100) ? 'pass' : pct >= 40 ? 'tesno' : 'fail'}">${sp.sansa > 0.99 ? L('visheOd99') : pct + '%'}</div>
      <div class="od10" aria-hidden="true">${Array.from({ length: 10 }, (x, i) => `<span class="${i < (pct >= 95 ? 9 : Math.round(pct / 10)) ? 'od10Da' : 'od10Ne'}"></span>`).join('')}</div>
      <p class="mut napomena">${escapeHtml(pct < 5 ? L('sansaOd10Malo') : pct >= 95 ? L('sansaOd10Skoro') : L('sansaOd10').split('@1').join(Math.round(pct / 10)))}</p>
      <button type="button" class="explCardBtn pojBtn" id="btnSansaKako">${L('sansaKakoDugme')}</button>
      <div class="explCard" style="display:none"><p class="mut napomena">${L('sansaKako').split('@1').join(sp.ukupno).split('@2').join(sp.prag)}</p></div>`}
      ...
      ${stavka(sp.nizOk, L('spremanNiz').split('@1').join(SIM_NIZ).split('@2').join(poenTxt(SIM_MARGINA)))}
      ...
      ${mapaIspitaHtml(readiness().slotovi)}
      <h3>${L('readyLoss')}</h3>
      ${rezervaHtml(exp, loss)}
      <table class="stats"><tbody>${top.map((t) =>
        `<tr class="statLink" data-sub="${t.subs[0]}"><td>${t.subs.map((s) => `<button type="button" class="repGo" data-sub="${s}" title="${escapeHtml(L('catOpen'))}">${escapeHtml(subShortName(s))}</button>`).join(' / ')}</td>
    ...
    // Klik bilo gde u redu i dalje radi; tastatura ide kroz dugme u prvoj ćeliji, koje
    // Enter i razmak dobija od pregledača — bez ručnog osmatrača i bez fokusabilnog <tr>.
    // mesto na testu sa više podoblasti: svaka ima svoje dugme (ranije je red nabrajao tri,
    // a otvarao samo prvu); klik van dugmeta i dalje otvara prvu
    { const bsk = el('btnSansaKako'); if (bsk) sklopivo(bsk, null, bsk.nextElementSibling); }
    el('readyCard').querySelectorAll('.mapaIspita .mapaPolje').forEach((b) => b.addEventListener('click', () => browse('s' + b.dataset.sub)));
    el('readyCard').querySelectorAll('.statLink').forEach((tr) => {
      tr.addEventListener('click', (ev) => {
        const b = ev.target.closest('.repGo');
        browse('s' + (b && b.dataset.sub ? b.dataset.sub : tr.dataset.sub));
      });
    });
  }
  ...
  function renderStats() {
    ...
    setHash('#/stats');
    renderReady();
    el('statsCard').innerHTML = `<h3>${L('statsTitle')}</h3><p class="mut napomena">${L('statsTip')}</p><div id="statsBars"></div><p class="mut napomena">${L('crticaLegenda')}</p>`;
    nacrtajOblasti(el('statsBars'), { tacnost: true });
    {
      const g = greskeIds();
      el('greskeCard').innerHTML = `<h3>${escapeHtml(L('greskeNaslov'))}</h3>
        <p class="mut napomena">${escapeHtml(g.ids.length ? L(g.dvaput ? 'greskeOpis' : 'greskeJednom').split('@1').join(nQ(g.ids.length)).split('@2').join(L(one(g.ids.length) ? 'naKojem' : 'naKojima')) : L('greskePrazno'))}</p>
  ...
  function daniBlok() {
    ...
    if (zaCrtez.length >= 2) {
      ...
      crtez = `<svg viewBox="0 0 ${W} ${H}" role="img" style="max-width:306px;width:100%;display:block;margin:8px auto"
        ...
        <line x1="4" y1="${prag}" x2="302" y2="${prag}" class="daniPrag" stroke-width="1.5" stroke-dasharray="5 4"/>
    ...
    const sazetak = zbirN
      ? `<p>${escapeHtml(L(one(nedelja.length) ? 'daniUkupnoJd' : few(nedelja.length) ? 'daniUkupnoPk' : 'daniUkupno').split('@1').join(one(nedelja.length) ? nedelja.length : uzBroj(nedelja.length, 'danJd', 'danPk', 'danMn')).split('@2').join(zbirN)
    ...
    return crtez + sazetak + `<table class="stats"><thead><tr><th>${L('daniDatum')}</th><th class="num">${L('daniNovih')}</th>
      ...
      <p class="mut napomena">${escapeHtml(L('daniKoloneNap'))}</p>`;
  }
  ...
  function browse(key) {
    ...
    let inQ = 0; const unseen = [], wrongNow = [];
    for (const qid of ids) {
      const r = S.q[qid];
      if (!(r && r.a)) unseen.push(qid);
      if (inQueue(qid)) { inQ++; wrongNow.push(qid); }
    }
    const { seen, acc } = tacnostSpiska(ids);
    ...
    head.innerHTML = `
      ...
      <div class="mut opisRed">${nQ(ids.length)} · ${seen} ${L('answered')}${acc !== null && seen >= 5 ? ` · ${L('posTacnoLbl')}: <span class="${accClass(acc)}">${acc}%</span>` : ''} · ${inQ} ${L('inQueue')}${pomocHtml()}</div>
      ${pomocTekstHtml(L('naIspituTip'))}
      <div class="qActions">
    ...
    if (type === 'c') {
      ...
      for (const sid of subIds) {
        ...
        const sAcc = sSeen >= 5 ? Math.round(100 * sGood / sSeen) : null;   // isto merilo kao u Statistici (i ista granica od 5)
        const b = document.createElement('button'); b.className = 'subRow skrijUPretrazi';
        b.title = subIme(sid);
        const naIsp = naIspitu(sid);
        b.innerHTML = `<span class="subName">${escapeHtml(subShortName(sid))}${naIsp ? ` <span class="subExam${naIsp === '0' ? ' subExamNula' : ''}" title="${escapeHtml(L('naIspituTip'))}">${naIsp === '0' ? L('naIspituNula') : L('naIspitu').replace('#', naIsp)}</span>` : ''}</span>
          <span class="subCnt">${sSeen}/${sq.length}</span>
          <span class="subAcc">${sAcc !== null ? `<span class="${accClass(sAcc)}">${sAcc}%</span>` : sSeen ? `<span class="mut">—<span class="samoZaCitace"> ${escapeHtml(L('maloPodataka'))}</span></span>` : ''}</span>`;
    ...
    crtajRedove(list, ids, {
      ...
      naslovOpis: (q) => subIme(q.sub),
      naKlik: (idx) => {
        if (shuffleOn) rowStart(ids, idx, secTitleFn(key), origin);
        else startList(ids, secTitleFn(key), null, 'section', { secKey: key, startAt: idx, origin, hash: '#/vezba/' + key });
      },
    });
    veziPretragu(list);
    if (type === 'c') {
  ...
  function browseAll() {
    ...
    for (const q of Q) {
      const r = S.q[q.id];
      if (!(r && r.a)) unseen.push(q.id);
      if (inQueue(q.id)) { inQ++; wrongNow.push(q.id); }
    }
    const { seen, acc } = tacnostSpiska(Q.map((q) => q.id));
    const head = el('browseHead');
    ...
    head.innerHTML = `<h3>${L('allPage')}</h3>
      <div class="mut opisRed">${nQ(Q.length)} · ${seen} ${L('answered')}${acc !== null && seen >= 5 ? ` · ${L('posTacnoLbl')}: <span class="${accClass(acc)}">${acc}%</span>` : ''} · ${inQ} ${L('inQueue')}${pomocHtml()}</div>
      ${pomocTekstHtml()}
      <div class="qActions">
        ${(() => {
    ...
    const list = el('browseList');
    list.hidden = false;   // prazno stanje „Obeležene" ga sakrije — ne sme da ostane sakriven ovde
    const imenaOblasti = [];
  ...
  function browseSet(setKind) {
    ...
    if (!ids.length && !(isWrong && stale.length)) {
      ...
      // ko nikad nije pogrešio nije ništa „grešio pa utvrdio" (revizija v148)
      const nikadGresio = !Q.some((q) => S.q[q.id] && S.q[q.id].w > 0);
      const poruka = isWrong ? (nista ? L('drillEmptyNovi') : nikadGresio ? L('drillEmptyBezGresaka') : L('drillEmpty')) : L('markedEmpty');
    ...
    head.innerHTML = `<h3>${escapeHtml(title)}</h3>
      <div class="mut opisRed">${isWrong
        ? `${ids.length} ${L('inQueue')}: ${ready.length} ${L(S.plan && S.plan.auto ? 'ponNaRedu' : 'ponDanas')} · ${waiting.length} ${L('ponKasnije')}${stale.length ? ` · ${stale.length} ${L('osveziLbl')}` : ''}${pomocHtml()}`
        : nQ(ids.length)}</div>
  ...
  const JE_IOS = /iPhone|iPad|iPod/.test(navigator.userAgent);
  // Napomena o čuvanju kaže ISTINU za ovaj pregledač (revizija v148): na iPhone-u Safari sam
  // briše podatke sajta posle 7 dana korišćenja bez posete, a ikona na početnom ekranu ima svoj,
  // odvojen napredak; bez File System Access nema „poveži fajl".
  const persistNoteTekst = () => L(JE_IOS && !jeStandalone() ? 'persistNoteIOS' : FSA ? 'persistNote' : 'persistNoteBezFSA');
  ...
  const TOUR_STEPS = [
    ...
    { sel: '#homeSummary .planBox', key: 'tourPlan' },
  ...
  function tourStart() {
    ...
    let idx = 0, spot = null;
    // samo koraci čiji element postoji (npr. bez okvira plana) — ranije je brojač skakao „1 / 8 → 3 / 8"
    const KORACI = TOUR_STEPS.filter((st) => document.querySelector(st.sel));
    ...
    const show = () => {
      ...
      tip.querySelector('.tourRow').innerHTML = `<span class="mut">${idx + 1} / ${KORACI.length}</span>
        <span style="flex:1"></span>
        <button type="button" class="secondary sBtn" id="tourSkip">${escapeHtml(L('tourSkip'))}</button>
        <button class="primary" id="tourNext">${escapeHtml(idx === KORACI.length - 1 ? L('tourDone') : L('tourNext'))}</button>`;
    ...
    const next = () => { idx++; if (idx >= KORACI.length) end(); else show(); };
    const onKey = (ev) => {
  ...
  function prviNeodgOd(pos) {
    const neodg = (i) => !S.q[Q[i].id] || !S.q[Q[i].id].a;
    for (let i = pos; i < Q.length; i++) if (neodg(i)) return i;
    // Iza pozicije nema neotvorenih: traži od početka. Do v147 je „Nastavi (1327/1327)" vodilo
    // pravo na kraj spiska, a neotvorena pitanja pre pozicije (kod Milana 403) nisu dolazila na red.
    for (let i = 0; i < Math.min(pos, Q.length); i++) if (neodg(i)) return i;
    return pos < Q.length ? pos : 0;   // sve otvoreno: drugi krug nastavlja gde je stao, posle kraja od 1.
  }
  ...
  // Promena datuma ispita ili režima menja osnovu računa: plan (plan.js) se računa iznova pri
  // sledećem crtanju. Keš se ionako menja sa svakim save(); ovo ga baca i pre toga.
  function ponistiAutoKvotu() { _planKes = null; }
  function planStanje() {
    if (!S.plan) return null;
    const d = (S.day && S.day.d === localDay()) ? S.day : null;
    const uNovih = d ? (d.novih || 0) : 0;
    const uPon = d ? (d.pon || 0) : 0;
    // RUČNI cilj: tvoji brojevi. Auto režim ima svoj plan (planBlokAuto → plan.js, v149) i ovde ne
    // ulazi — stari auto (kvota iz roka za novo gradivo) je uklonjen, da ne postoje dva modela.
    const cNovih = S.plan.novih || 0;
    const cPon = S.plan.pon || 0;
    return {
      auto: false,
      pod: false,
      autoBezDatuma: false,
  ...
  // ---------- Crteži uz plan i spremnost (v150) ----------
  // Revizija v149: prosečan korisnik nije povezivao „danas ≈67 (4%)" sa „na dan ispita ≈85 (58%)" —
  // dva broja, dve šanse, bez slike. Skala 0–98 sa pragom 84 pokazuje oba mesta odjednom. Sve što crtež
  // kaže stoji i rečima (aria-label i natpis ispod), jer čitač ekrana ne vidi tačke.
  const bojaOcene = (o) => (o === 'ok' ? 'var(--ok)' : o === 'upozorenje' ? 'var(--mark)' : o === 'problem' ? 'var(--bad)' : 'var(--blue)');
  // „Na dan ispita" na skali i u rečenici je uvek „ako nastaviš ovim tempom" — i posle pauze, kad presuda
  // plana gleda šta bi bilo bez vežbe (projekcija.poeni je tada „bez vežbe", a natpis kaže „ovim tempom").
  // procenat šanse iz plana — isti zapis kao u tekstu plana (plan.js „pr": 69,6% je „69%", ne „70%", jer
  // zaokruživanje ne sme da pređe prag presude; nikad „100%" ni „0%")
  const prPlan = (p) => (self.VozackiPlan && self.VozackiPlan.pr ? self.VozackiPlan.pr(p) : Math.round(100 * p) + '%');
  function ispitNastavak(r) {
    const pr = (r && r.projekcija) || {};
    const E = Number.isFinite(pr.poeniNastavak) ? pr.poeniNastavak : pr.poeni;
    const P = Number.isFinite(pr.sansaNastavak) ? pr.sansaNastavak : pr.sansa;
    const ocena = !r || r.ocena === 'nema' || !Number.isFinite(E) ? 'nema' : !Number.isFinite(P) ? r.ocena : P >= 0.7 ? 'ok' : P >= 0.4 ? 'upozorenje' : 'problem';
    return { E, P, ocena };
  }
  // pojas „manje / više rada" na dan ispita: od najmanjeg do najvećeg od tri broja (projekcija ume da
  // ih zameni za desetinku — to je šum, a pojas ne sme da ispadne naopako)
  function pojasIspita(r) {
    const pu = (r && r.putanja) || {}, e = ispitNastavak(r).E;
    const n1 = (x) => (Array.isArray(x) && x.length ? x[x.length - 1] : undefined);
    const v = [n1(pu.Emanje), e, n1(pu.Evise)].filter(Number.isFinite);
    return v.length >= 2 ? { manje: Math.min(...v), vise: Math.max(...v) } : {};
  }
  function skalaSvg(p) {
    const vr = [p.danas, p.ispit, p.manje, p.vise].filter(Number.isFinite);
    if (!vr.length) return '';
    const levo = 14, W = 306, desno = W - 14, y = 44, PRAG = prag(SIM_PTS_MIN);
    const od = Math.max(0, Math.min(60, Math.floor((Math.min(...vr) - 6) / 10) * 10));
    const X = (v) => levo + (desno - levo) * (Math.min(SIM_PTS_MIN, Math.max(od, v)) - od) / (SIM_PTS_MIN - od);
    const r1 = (v) => Math.round(v);
    const boja = bojaOcene(p.ocena);
    const imaI = Number.isFinite(p.ispit), imaD = Number.isFinite(p.danas);
    const blizu = imaI && imaD && Math.abs(X(p.ispit) - X(p.danas)) < 64;
    const H = blizu ? 94 : 78;
    const opis = (imaD ? L('skalaDanas').split('@1').join(poenTxt(r1(p.danas))) : '')
      + (imaI ? (imaD ? ' → ' : '') + L('skalaIspit').split('@1').join(r1(p.ispit)) : '')
      + ' ' + L('skalaPrag').split('@1').join(PRAG)
      + (Number.isFinite(p.manje) && Number.isFinite(p.vise) && r1(p.manje) !== r1(p.vise) ? ' ' + L('skalaPojas').split('@1').join(r1(p.manje)).split('@2').join(r1(p.vise)) : '');
    return `<svg class="skala" viewBox="0 0 ${W} ${H}" role="img" aria-label="${escapeHtml(opis)}">
      <line class="skStaza" x1="${levo}" y1="${y}" x2="${desno}" y2="${y}"/>
      <line class="skProlaz" x1="${X(PRAG).toFixed(1)}" y1="${y}" x2="${desno}" y2="${y}"/>
      <line class="skPrag" x1="${X(PRAG).toFixed(1)}" y1="${y - 12}" x2="${X(PRAG).toFixed(1)}" y2="${y + 12}"/>
      <text class="skOsa" x="${X(PRAG).toFixed(1)}" y="${y + 26}" text-anchor="middle">${PRAG}</text>
      <text class="skOsa" x="${levo}" y="${y + 26}">${od}</text>
      <text class="skOsa" x="${desno}" y="${y + 26}" text-anchor="end">${SIM_PTS_MIN}</text>
      ${Number.isFinite(p.manje) && Number.isFinite(p.vise) ? `<rect x="${X(p.manje).toFixed(1)}" y="${y - 7}" width="${Math.max(2, X(p.vise) - X(p.manje)).toFixed(1)}" height="14" rx="7" style="fill:${boja};opacity:.28"/>` : ''}
      ${imaD && imaI ? `<line class="skStrela" x1="${X(p.danas).toFixed(1)}" y1="${y}" x2="${X(p.ispit).toFixed(1)}" y2="${y}"/>` : ''}
      ${imaI ? `<circle cx="${X(p.ispit).toFixed(1)}" cy="${y}" r="8" style="fill:${boja}"/><text class="skNatpis skJak" x="${Math.min(desno - 34, Math.max(levo + 34, X(p.ispit))).toFixed(1)}" y="${y - 14}" text-anchor="middle">${escapeHtml(L('skalaIspitKratko'))} ≈${r1(p.ispit)}</text>` : ''}
      ${imaD ? `<circle cx="${X(p.danas).toFixed(1)}" cy="${y}" r="5" class="skDanas"/><text class="skNatpis" x="${Math.min(desno - 30, Math.max(levo + 30, X(p.danas))).toFixed(1)}" y="${blizu ? y + 44 : y - 14}" text-anchor="middle">${escapeHtml(L('skalaDanasKratko'))} ≈${r1(p.danas)}</text>` : ''}
    </svg><div class="mut napomena skalaTekst" aria-hidden="true">${escapeHtml(opis)}</div>`;
  }
  // „Tvoj tempo": poslednjih 7 dana (i danas) — koliko je urađeno, i linija sa koliko plan računa.
  function tempoSvg(kap) {
    const arh = Array.isArray(S.dani) ? S.dani : [];
    const d = (S.day && S.day.d === localDay()) ? [{ ...S.day, tekuci: 1 }] : [];
    const dani = [...arh, ...d].slice(-8);
    if (dani.length < 2) return '';
    const W = 306, H = 120, dno = 92, vrh = 16;
    const maks = Math.max(kap || 0, ...dani.map((x) => x.n || 0), 10);
    const Y = (v) => dno - (dno - vrh) * v / maks;
    const sir = Math.min(30, Math.floor((W - 20) / dani.length) - 6);
    const kor = (W - 20 - sir * dani.length) / Math.max(1, dani.length - 1);
    const stubovi = dani.map((x, i) => {
      const x0 = Math.round(10 + i * (sir + kor)), h = dno - Y(x.n || 0);
      return `<rect x="${x0}" y="${(dno - h).toFixed(1)}" width="${sir}" height="${h.toFixed(1)}" rx="3" class="${x.tekuci ? 'tpDanas' : 'tpStub'}"/>`
        + `<text class="skOsa" x="${x0 + sir / 2}" y="${(dno - h - 4).toFixed(1)}" text-anchor="middle">${x.n || 0}</text>`
        + `<text class="skOsa" x="${x0 + sir / 2}" y="${dno + 14}" text-anchor="middle">${x.d.slice(8)}.${x.d.slice(5, 7)}.</text>`;
    }).join('');
    const opis = L('tempoOpis').split('@1').join(dani.map((x) => x.n || 0).join(', ')).split('@2').join(kap);
    return `<svg class="skala" viewBox="0 0 ${W} ${H}" role="img" aria-label="${escapeHtml(opis)}">
      ${stubovi}
      ${kap ? `<line class="skPrag" x1="6" y1="${Y(kap).toFixed(1)}" x2="${W - 6}" y2="${Y(kap).toFixed(1)}"/><text class="skNatpis" x="${W - 6}" y="${(Y(kap) - 4).toFixed(1)}" text-anchor="end">${escapeHtml(L('tempoPlan').split('@1').join(kap))}</text>` : ''}
      <line class="skStaza2" x1="6" y1="${dno}" x2="${W - 6}" y2="${dno}"/></svg><div class="mut napomena skalaTekst">${escapeHtml(L('tempoNatpis'))}</div>`;
  }
  // Rezultati pravih simulacija kroz vreme, uz prag 84.
  function simSvg() {
    const prave = S.sims.filter(pravaSim).slice(-12);
    if (prave.length < 2) return '';
    const W = 306, H = 120, levo = 26, desno = W - 10, vrh = 12, dno = 96, PRAG = prag(SIM_PTS_MIN);
    // početak ose blizu najslabijeg rezultata (ali ispod praga), da se razlike vide
    const od = Math.max(0, Math.min(PRAG - 4, Math.floor((Math.min(...prave.map((s) => s.score)) - 4) / 5) * 5));
    const X = (i) => levo + (desno - levo) * i / (prave.length - 1);
    const Y = (v) => dno - (dno - vrh) * (v - od) / (SIM_PTS_MIN - od);
    const tacke = prave.map((s, i) => X(i).toFixed(1) + ',' + Y(s.score).toFixed(1)).join(' ');
    const opis = L('simGrafOpis').split('@1').join(prave.map((s) => s.score).join(', ')).split('@2').join(PRAG);
    return `<svg class="skala" viewBox="0 0 ${W} ${H}" role="img" aria-label="${escapeHtml(opis)}">
      <line class="skPrag" x1="${levo}" y1="${Y(PRAG).toFixed(1)}" x2="${desno}" y2="${Y(PRAG).toFixed(1)}"/>
      <text class="skOsa" x="${levo - 4}" y="${(Y(PRAG) + 4).toFixed(1)}" text-anchor="end">${PRAG}</text>
      <text class="skOsa" x="${levo - 4}" y="${(Y(SIM_PTS_MIN) + 4).toFixed(1)}" text-anchor="end">${SIM_PTS_MIN}</text>
      <polyline class="skLinija" points="${tacke}"/>
      ${prave.map((s, i) => `<circle cx="${X(i).toFixed(1)}" cy="${Y(s.score).toFixed(1)}" r="4" style="fill:${s.score >= PRAG ? 'var(--ok)' : 'var(--bad)'}"/>`).join('')}
      <text class="skNatpis skJak" x="${X(prave.length - 1).toFixed(1)}" y="${(Y(prave[prave.length - 1].score) - 8).toFixed(1)}" text-anchor="end">${prave[prave.length - 1].score}</text>
      <text class="skOsa" x="${levo}" y="${dno + 16}">${fmtDatum(prave[0].d).slice(0, 6)}</text>
      <text class="skOsa" x="${desno}" y="${dno + 16}" text-anchor="end">${fmtDatum(prave[prave.length - 1].d).slice(0, 6)}</text>
    </svg><div class="mut napomena skalaTekst">${escapeHtml(L('simGrafNatpis'))}</div>`;
  }
  let planDetaljiOtvoreno = false;
  // Putanja do ispita: očekivani poeni dan po dan ovim tempom (puna linija), pojas „manje/više rada",
  // isprekidano „bez vežbe" (slika „sve pa ništa"), prag 84. Kad plan kaže da je malo dokaza — sivo.
  function putanjaSvg(p, nat) {
    if (!p || !Array.isArray(p.E) || p.E.length < 2) return '';
    const W = 306, H = 150, levo = 30, desno = W - 10, vrh = 14, dno = 112, n = p.E.length;
    const nizovi = [p.E, p.Emanje, p.Evise, p.Ebez].filter((x) => Array.isArray(x) && x.length === n);
    const sve = nizovi.flat().concat([p.prag]).filter(Number.isFinite);
    const od = Math.max(0, Math.min(p.prag - 6, Math.floor((Math.min(...sve) - 4) / 5) * 5));
    const X = (i) => levo + (desno - levo) * i / (n - 1);
    const Y = (v) => dno - (dno - vrh) * (Math.min(SIM_PTS_MIN, Math.max(od, v)) - od) / (SIM_PTS_MIN - od);
    const tacke = (x) => x.map((v, i) => X(i).toFixed(1) + ',' + Y(v).toFixed(1)).join(' ');
    const imaPojas = Array.isArray(p.Emanje) && Array.isArray(p.Evise) && p.Emanje.length === n && p.Evise.length === n;
    // pojas od najmanjeg do najvećeg od tri broja po danu (šum projekcije ume da ih zameni za desetinku)
    const gore = imaPojas ? p.E.map((v, i) => Math.max(v, p.Emanje[i], p.Evise[i])) : null, dole = imaPojas ? p.E.map((v, i) => Math.min(v, p.Emanje[i], p.Evise[i])) : null;
    const pojas = imaPojas ? `<polygon class="ptPojas" points="${tacke(gore)} ${dole.map((v, i) => X(n - 1 - i).toFixed(1) + ',' + Y(dole[n - 1 - i]).toFixed(1)).join(' ')}"/>` : '';
    const kratak = (ds) => ds.slice(8) + '.' + ds.slice(5, 7) + '.';
    // natpisi iz istih brojeva kao skala iznad (ranije: 75,456 → „75" na skali, a 75,5 → „76" na putanji)
    const e0 = Math.round(nat && Number.isFinite(nat.danas) ? nat.danas : p.E[0]), eN = Math.round(nat && Number.isFinite(nat.ispit) ? nat.ispit : p.E[n - 1]);
    const opis = L('putanjaOpis').split('@1').join(poenTxt(e0)).split('@2').join(poenTxt(eN)).split('@3').join(p.prag);
    return `<svg class="skala putanja${p.pouzdano === false ? ' nepouzdano' : ''}" viewBox="0 0 ${W} ${H}" role="img" aria-label="${escapeHtml(opis)}">
      ${pojas}
      <line class="skPrag" x1="${levo}" y1="${Y(p.prag).toFixed(1)}" x2="${desno}" y2="${Y(p.prag).toFixed(1)}"/>
      <text class="skOsa" x="${levo - 4}" y="${(Y(p.prag) + 4).toFixed(1)}" text-anchor="end">${p.prag}</text>
      ${Array.isArray(p.Ebez) && p.Ebez.length === n ? `<polyline class="ptBez" points="${tacke(p.Ebez)}"/>` : ''}
      <polyline class="skLinija" points="${tacke(p.E)}"/>
      <text class="skNatpis skJak" x="${X(n - 1) - 4}" y="${(Y(p.E[n - 1]) - 8).toFixed(1)}" text-anchor="end">≈${eN}</text>
      <line class="skStaza2" x1="${levo}" y1="${dno}" x2="${desno}" y2="${dno}"/>
      <text class="skOsa" x="${levo}" y="${dno + 15}">${escapeHtml(L('putanjaDanas'))}</text>
      <text class="skOsa" x="${desno}" y="${dno + 15}" text-anchor="end">${escapeHtml(L('putanjaIspit'))} ${kratak(p.dani[n - 1])}</text>
    </svg><div class="mut napomena skalaTekst"><span aria-hidden="true">${escapeHtml(opis)}</span> ${escapeHtml(L(p.pouzdano === false ? 'putanjaPrva' : 'putanjaLegenda'))}</div>`;
  }
  // Sva pitanja u četiri dela: viđeno | stižeš do ispita | ostaju (i koliko ih dolazi na ispit) | van šablona.
  // Zbir je uvek 1327 — traka pokazuje cenu neviđenih umesto reči „retko".
  function pokrivenostHtml(p) {
    if (!p || !Number.isFinite(p.videno)) return '';
    const uk = p.videno + p.stize + p.retka + p.vanSablona;
    if (!uk) return '';
    const w = (v) => (100 * v / uk).toFixed(2) + '%';
    const naIsp = Number.isFinite(p.retkaNaIspitu) ? Math.round(p.retkaNaIspitu) : null;
    const opis = L('pokrOpis').split('@1').join(p.videno).split('@2').join(p.stize).split('@3').join(p.retka).split('@4').join(p.vanSablona)
      + (p.retka && naIsp !== null ? ' ' + L(naIsp >= 1 ? 'pokrNaIspitu' : 'pokrNaIspituMalo').split('@1').join(naIsp) : '');
    return `<div class="pokr" role="img" aria-label="${escapeHtml(opis)}"><span class="pkVid" style="width:${w(p.videno)}"></span><span class="pkStize" style="width:${w(p.stize)}"></span><span class="pkRetka" style="width:${w(p.retka)}"></span><span class="pkVan" style="width:${w(p.vanSablona)}"></span></div>
      <div class="mut napomena pkLeg" aria-hidden="true">${[['pkVid', 'pkLVid', p.videno], ['pkStize', 'pkLStize', p.stize], ['pkRetka', 'pkLRetka', p.retka], ['pkVan', 'pkLVan', p.vanSablona]].map(([k, lk, v]) => `<span class="pkStavka"><span class="pkSw ${k}"></span>${escapeHtml(L(lk))} ${v}</span>`).join(' ')}${p.retka && naIsp !== null ? ' ' + escapeHtml(L(naIsp >= 1 ? 'pokrNaIspitu' : 'pokrNaIspituMalo').split('@1').join(naIsp)) : ''}</div>`;
  }
  // „Šta ako": dnevni obim → šansa na dan ispita (plan.js, na zahtev), uz oznaku tvog i potrebnog tempa.
  function stoAkoHtml(s) {
    if (!Array.isArray(s) || !s.length) return `<p class="mut napomena">${escapeHtml(L('stoAkoNema'))}</p>`;
    const red = (x) => {
      const pct = x.P == null ? null : Math.round(100 * x.P);
      const oznaka = x.tvoj ? L('stoAkoTvoj') : x.potreban ? L('stoAkoPotreban') : '';
      return `<div class="planRed stoRed${x.tvoj ? ' stoTvoj' : ''}"><span class="planIme">${escapeHtml(L('stoAkoRed').split('@1').join(x.dnevno))}${oznaka ? ` <span class="mut">(${escapeHtml(oznaka)})</span>` : ''}</span>
        <span class="planBar"><span class="${pct === null ? '' : x.P >= 0.7 ? 'barDobar' : x.P >= 0.4 ? 'barSrednji' : 'barLos'}" style="width:${pct === null ? 0 : pct}%"></span></span>
        <span class="mut">${x.P == null ? '—' : prPlan(x.P).replace('preko ', '>').replace('ispod ', '<')}</span></div>`;
    };
    return s.slice().sort((x, y) => x.dnevno - y.dnevno).map(red).join('') + `<p class="mut napomena">${escapeHtml(L('stoAkoObjasnjenje'))}</p>`;
  }
  // Rezerva poena (revizija v149, vizuali): „Najviše te košta" pokazuje oko trećine gubitka; traka
  // pokazuje ceo: očekivani poeni, pet najskupljih mesta, ostalo, i koliko je to od praga.
  function rezervaHtml(exp, loss) {
    if (!Number.isFinite(exp) || !loss) return '';
    const MAX = SIM_PTS_MIN, PRAG = prag(MAX);
    const top = Object.values(loss).sort((x, y) => y.pts - x.pts).slice(0, 5);
    const gub = Math.max(0, MAX - exp), topZbir = top.reduce((s, t) => s + t.pts, 0), ostalo = Math.max(0, gub - topZbir);
    const w = (v) => (100 * v / MAX).toFixed(2) + '%';
    const delovi = `<span class="rzZna" style="width:${w(exp)}" title="≈ ${Math.round(exp)}"></span>`
      + top.map((t, i) => `<span class="rzGub" style="width:${w(t.pts)};opacity:${(1 - i * 0.14).toFixed(2)}" title="${escapeHtml(t.subs.map((s) => subShortName(s)).join(' / '))}: −${t.pts.toFixed(1).replace('.', ',')}"></span>`).join('')
      + `<span class="rzOst" style="width:${w(ostalo)}"></span>`;
    const razlika = Math.round(Math.abs(exp - PRAG));
    const opis = exp >= PRAG ? L('rezervaIznad').split('@1').join(poenTxt(razlika)).split('@2').join(PRAG) : L('rezervaIspod').split('@1').join(poenTxt(razlika)).split('@2').join(PRAG);
    return `<div class="rezerva" role="img" aria-label="${escapeHtml(opis)}"><div class="rzTraka">${delovi}</div><span class="rzPrag" style="left:${w(PRAG)}"></span></div>
      <p class="mut napomena"><span aria-hidden="true">${escapeHtml(opis)}</span> ${escapeHtml(L('rezervaLegenda'))}</p>`;
  }
  // Mapa ispita (revizija v149, vizuali): šablon testa — 41 mesto po oblastima — i koliko je
  // verovatno da svako pogodiš danas. Jedini prikaz šablona; pokazuje i gde se gube poeni.
  function mapaIspitaHtml(slotovi) {
    if (!Array.isArray(slotovi) || slotovi.length !== SIM_SLOTS.length) return '';
    const catOfSub = new Map(Q.map((q) => [q.sub, q.cat]));
    const grupe = new Map();
    slotovi.forEach((s, i) => {
      const cat = catOfSub.get(s.subs[0]);
      if (!grupe.has(cat)) grupe.set(cat, []);
      grupe.get(cat).push({ ...s, i });
    });
    const boja = (p) => (p >= 0.9 ? 'mp4' : p >= 0.8 ? 'mp3' : p >= 0.65 ? 'mp2' : 'mp1');
    // oblasti sa po jednim mestom idu u jedan red (inače sedam redova od po jednog polja)
    const velike = [...grupe.entries()].filter(([, sl]) => sl.length > 1);
    const male = [...grupe.values()].filter((sl) => sl.length === 1).flat();
    const sveGrupe = velike.concat(male.length ? [['male', male]] : []);
    const polja = sveGrupe.map(([cat, sl]) => `<div class="mapaGrupa"><div class="mut mapaIme">${cat === 'male' ? escapeHtml(L('mapaMale').split('@1').join(sl.length)) : escapeHtml(T(catName.get(cat))) + ' · ' + sl.length}</div><div class="mapaPolja">`
      + sl.map((s) => `<button type="button" class="mapaPolje ${boja(s.p)}" data-sub="${s.subs[0]}" title="${escapeHtml(s.subs.map((x) => subShortName(x)).join(' / '))} · ${Math.round(100 * s.p)}%" aria-label="${escapeHtml(s.subs.map((x) => subShortName(x)).join(' / '))}: ${s.pts} ${escapeHtml(L('points'))}, ${Math.round(100 * s.p)}%">${s.pts}</button>`).join('')
      + '</div></div>').join('');
    return `<h3>${L('mapaNaslov')}</h3><p class="mut napomena">${L('mapaOpis')}</p><div class="mapaIspita">${polja}</div>
      <p class="mut napomena mapaLegenda"><span class="mapaPolje mp4"></span> ${L('mapaL4')} <span class="mapaPolje mp3"></span> ${L('mapaL3')} <span class="mapaPolje mp2"></span> ${L('mapaL2')} <span class="mapaPolje mp1"></span> ${L('mapaL1')}</p>`;
  }

  // ---------- Dnevni plan u auto režimu (plan.js, v149) ----------
  // Stari auto je računao „koliko TREBA" (rok za novo gradivo, pa kvote) i nije znao koliko čovek
  // stiže: mereno (tools/plan-proba.mjs) tražio je više od stvarnog kapaciteta 61–96% dana, do ×48.
  // Nov plan (plan.js) traži koliko stižeš u 4 od 5 dana i bira po vrednosti na ispitu; presuda je
  // projekcija do dana ispita. Računa se 15–70 ms, pa se pamti dok se stanje ne promeni.
  let _planKes = null;
  function planAuto(jos, stoAko) {
    const P = window.VozackiPlan;
    if (!P || !S.plan || !S.plan.auto) return null;
    const dana = danaDoIspita();
    if (dana === null || dana < 0) return null;
    const minKap = S.plan.pod ? (S.plan.novih || 0) + (S.plan.pon || 0) : 0;
    // ključ: brojač čuvanja + otisak stanja od koga plan zavisi — i promena mimo save() (uvoz, drugi
    // prozor, provera) računa plan iznova umesto da vrati stari
    const kljuc = () => [verzijaStanja, localDay(), jos || 0, stoAko ? 1 : 0, minKap, S.examDate, S.day && S.day.d, S.day && S.day.n, S.sims.length,
      (S.dani || []).length, Object.keys(S.q).length, JSON.stringify(S.plan)].join('|');
    if (_planKes && _planKes.s === S && _planKes.k === kljuc()) return _planKes.r;
    const danas = localDay();
    const bioZapis = !!(S.plan.prognoza && S.plan.prognoza.tr && S.plan.prognoza.tr[danas]);
    let r = null;
    try {
      r = P.planDanas({ get S() { return S; }, localDay, danaDoIspita, queueSplit, zaOsvezavanje, inQueue, dueOf, Q, byId },
        { sad: Date.now(), Q, byId, SIM_SLOTS, jos: jos || 0, minKap, stoAko: !!stoAko });
    } catch (e) { console.warn('Plan nije mogao da se izračuna:', e); r = null; }
    // prvi račun u danu upisuje koliko je plan tražio — sutra se iz toga vidi da li je stignuto
    if (!bioZapis && S.plan.prognoza && S.plan.prognoza.tr && S.plan.prognoza.tr[danas]) save();
    _planKes = { s: S, k: kljuc(), r };
    return r;
  }
  function planBlokAuto() {
    const glava = `<div class="planBox"><h3 class="planNaslov">${L('planNaslovAuto')}</h3> &nbsp;<button type="button" class="bcLink" id="btnPlanPodesi">${L('planPodesiAuto')} ›</button>`;
    const dana = danaDoIspita();
    if (dana === null) return planPozivHtml(true);
    if (dana < 0) return glava + `<div class="mut napomena">${L('prosaoDatum').split('@1').join(fmtDatum(S.examDate))}</div></div>`;
    const r = planAuto();
    if (!r) return glava + `<div class="mut napomena">${L('planNijeIzracunat')}</div></div>`;
    const d = (S.day && S.day.d === localDay()) ? S.day : null;
    const n = d ? (d.n || 0) : 0;
    // cilj trake daje plan: koliko obično stižeš (sa simulacijom), na dan ispita samo kratko ponavljanje,
    // a kad ništa nije na redu cilja nema — ranije je tada stajalo „0 / 60 · ostaje 60" (nezavisna provera)
    const cilj = r.cilj == null ? null : Math.max(1, r.cilj);
    // traka iz delova (revizija v149): urađeno | lista koja je ostala | simulacija — zbir je današnji cilj,
    // pa „ostaje" uvek ima ime (ranije: „ostaje 62" iznad dugmeta „(21)")
    const lista = r.ostaje ? r.ostaje.lista : r.ids.length, sim = r.ostaje ? r.ostaje.simulacija : (r.rezerva || 0), uk = n + lista + sim;
    const pct = (x) => (uk ? (100 * x / uk).toFixed(1) : 0);
    const traka = cilj === null ? `<div class="planRed"><span class="planIme">${L('planDanasUk')}: <b>${n}</b></span></div>` : `<div class="planRed"><span class="planIme">${L('planDanasUk')}: <b>${n}</b> / ${uk}</span>
      <span class="planBar planBarDelovi"><span style="width:${pct(n)}%"></span><span class="pbLista" style="width:${pct(lista)}%"></span><span class="pbSim" style="width:${pct(sim)}%"></span></span>
      <span class="mut">${lista + sim === 0 ? '✓' : ''}</span></div>
      ${lista + sim && (n > 0 || sim > 0) ? `<div class="mut napomena planDelovi">${escapeHtml(L('planDeloviTxt').split('@1').join(n).split('@2').join(lista)) + (sim ? escapeHtml(L('planDeloviSim').split('@1').join(sim)) : '')}</div>` : ''}`;
    // Revizija v149: okvir je bio zid od 132–166 reči i 25–30 brojeva, a dugme ispod ekrana na 125%.
    // Sada: presuda i današnji zadatak gore, dugme odmah ispod, skala „danas → dan ispita", a sve
    // ostalo iza „Zašto baš ovoliko? ›". Nov plan.js daje presudaIdx/zadatak; stari samo redove.
    const R = r.redovi || [];
    const iP = Number.isInteger(r.presudaIdx) ? r.presudaIdx : R.findIndex((x) => /^(✅|⚠|⛔|Šansu|Prva)/.test(x.l));
    const iZ = r.zadatak ? R.findIndex((x, i) => i !== iP && x.l === r.zadatak) : R.findIndex((x, i) => i !== iP && /^(Danas|Lista za danas|Za danas)/.test(x.l));
    const ostalo = R.filter((x, i) => i !== iP && i !== iZ);
    const gore = (iP >= 0 ? `<div class="planPresuda">${escapeHtml(T(R[iP]))}</div>` : '')
      + (iZ >= 0 ? `<div class="planZadatak">${escapeHtml(T(R[iZ]))}</div>` : '');
    const rd = readiness();
    // S3: na dan ispita „danas" i „ispit" su isto — skala crta samo danas
    const nast = ispitNastavak(r), pojas = dana > 0 && nast.ocena !== 'nema' ? pojasIspita(r) : {};
    const skala = skalaSvg({
      danas: rd.exp !== null ? rd.exp : undefined,
      ispit: dana > 0 && nast.ocena !== 'nema' && Number.isFinite(nast.E) ? nast.E : undefined,
      manje: pojas.manje,
      vise: pojas.vise,
      ocena: nast.ocena,
    });
    const detalji = ostalo.map((x) => `<div class="mut napomena">${escapeHtml(T(x))}</div>`).join('')
      // putanja samo uz dokaze (ili „prvu procenu" sa malo dokaza) — bez njih plan ne daje ni poene ni šansu,
      // a crtež je korisniku sa 0 odgovora pokazivao „danas ≈ 74 poena"
      + (dana > 0 ? (r.ocena !== 'nema' || r.tanko ? putanjaSvg(r.putanja, { danas: rd.exp, ispit: nast.E }) : '') + pokrivenostHtml(r.pokrivenost) + tempoSvg(r.projekcija && Number.isFinite(r.projekcija.tempo) ? r.projekcija.tempo : r.kap) : '')
      + (dana > 0 && r.ocena !== 'nema' ? `<div class="razmakG"><button type="button" class="secondary sBtn" id="btnStoAko">${L('stoAkoDugme')}</button><div id="stoAkoTelo"></div></div>` : '');
    const ima = r.ids.length;
    const dno = ima ? `<button class="primary" id="btnPlanVezbaj">${L('planVezbaj')} (${ima})</button>`
      : r.nemaNaRedu ? '' // tekst plana već kaže da danas nema ničega na redu — ni ✅ ni „još 20"
      : (r.rezerva ? `<button type="button" class="primary" data-nav="sim">${L('endSimBtn')}</button> ` : `<span class="mut">${L('planAutoGotovo')}</span> `)
        + `<button type="button" class="secondary sBtn" id="btnPlanJos">${L('planJos')}</button>`;
    return glava + gore + traka + tacnostDanas() + `<div class="razmakG">${dno}</div>` + skala
      + (detalji ? `<button type="button" class="explCardBtn pojBtn razmakG" id="btnPlanDetalji">${L('planZasto')}</button>
        <div class="explCard" id="planDetaljiTelo"${planDetaljiOtvoreno ? '' : ' style="display:none"'}>${detalji}</div>` : '') + '</div>';
  }
  // „Još 20": posle ispunjenog plana, sledeća pitanja iz ISTOG poretka po vrednosti — dodatak koji
  // sam podiže procenu kapaciteta (dan kad uradiš više od plana kaže „mogu bar ovoliko").
  function planJos() {
    const r = planAuto(20);
    const ids = r ? r.ids.slice(-20) : [];
    if (!ids.length) { poruci(L('planNemaSta')); return; }
    startList(ids, () => L('planNaslovAuto') + ' — ' + L('planJosKratko'), () => L('planNemaSta'), 'filter', { origin: () => renderHome(), plan: true });
  }

  function planIds() {
    if (S.plan && S.plan.auto) { const r = planAuto(); return r ? r.ids : []; }
    const p = planStanje();
    if (!p) return [];
    let nova = [];
  ...
  // Poziv za dnevni plan na početnoj: nov korisnik ga nije nalazio (revizija v149 — ni tura, ni vodič,
  // ni početna ga nisu pominjali). Datum se upisuje odmah tu; „Ne treba mi plan" ga sklanja.
  function planPozivHtml(bezDatuma) {
    return `<div class="planBox planPoziv"><h3 class="planNaslov">${L(bezDatuma ? 'planNaslovAuto' : 'planPozivNaslov')}</h3>
      <div class="mut napomena">${L(bezDatuma ? 'autoBezDatuma' : 'planPozivOpis')}</div>
      <div class="planPozivRed"><label>${L('planPozivDatum')} <input type="date" id="planPozivDatum" value="${S.examDate || ''}"></label></div>
      <div class="qActions"><button type="button" class="primary" id="btnPlanUkljuci">${L(bezDatuma ? 'planPozivSacuvaj' : 'planPozivDugme')}</button>
      ${bezDatuma ? '' : `<button type="button" class="secondary sBtn" id="btnPlanNe">${L('planPozivNe')}</button>`}</div></div>`;
  }
  function planBlok() {
    if (!S.plan) return S.planNe ? '' : planPozivHtml(false);
    if (S.plan && S.plan.auto) return planBlokAuto();
    const p = planStanje();
    if (!p) return '';
    // plan koji ima samo prioritet (bez kvota i bez auto): nema šta da se meri, pa nema ni
    // trake ni „cilj ispunjen" — samo podsetnik da je prioritet uključen
    if (!S.plan.auto && p.cNovih <= 0 && p.cPon <= 0) {
      // Uključen prekidač bez brojeva: nema šta da se meri, ali se KAŽE da je uključen —
      // inače plan postoji u stanju, guši podsetnike na početnoj, a nigde se ne vidi.
      const redovi = [];
      if (S.plan.prio) redovi.push(L('lostPrioUkljucen'));
      if (S.plan.pod) redovi.push(L('podBezAuto'));
      return redovi.length
        ? `<div class="planBox"><h3 class="planNaslov">${L('planNaslov')}</h3> &nbsp;<button type="button" class="bcLink" id="btnPlanPodesi">${L('planPodesi')} ›</button>
           ${redovi.map((x) => `<div class="mut napomena">${x}</div>`).join('')}</div>`
        : '';
    }
    ...
    const visak = (p.cNovih > 0 && p.uNovih > p.cNovih)
      ? `<div class="mut napomena">${L('viskDanas').split('@1').join(uzBroj(p.uNovih, 'novoP', 'novaP', 'novihP')).split('@2').join(p.cNovih).split('@3').join(p.uNovih - p.cNovih)}</div>` : '';
    ...
    return `<div class="planBox"><h3 class="planNaslov">${L('planNaslov')}</h3> &nbsp;<button type="button" class="bcLink" id="btnPlanPodesi">${L('planPodesi')} ›</button>
  ...
  function homeExtras() {
    ...
    // broj za ponavljanje danas stoji u sažetku iznad (sa imenom i objašnjenjem) — ovde ne ide
    // dvaput, jedan ispod drugog (revizija v148)
    {
      ...
      // na sam dan ispita i posle njega nema predloga simulacije: tada stoji „ispit je danas"
      // ili poruka o prošlom datumu (ranije: „poslednja nedelja" i uveče posle ispita)
      if (doIspita !== null && doIspita < 1) {
        // bez predloga
      } else if (doIspita !== null && doIspita <= 7) {
    ...
    if (S.examDate) {
      ...
      if (dana !== null && dana >= 0) {
        ...
        const neodg = neodgovorenih();
        if (dana > 0 && neodg > 0) {
          // Predlog stoji SAMO dok plana nema. Kad plan postoji, o tempu sudi blok „Dnevni
          // cilj" — on broji i zaostali red, a njegovo dugme upisuje oba broja. Ranije je ista
          // opomena stajala dvaput, sa dva dugmeta koja upisuju različit cilj.
          // bez broja: stari predlog je delio gradivo na dane do roka i nije znao koliko stižeš
          // (kasno počet: stotine novih dnevno). Plan koji to zna je jedan prekidač dalje.
          // (v150: poziv za plan je kartica ispod — ne ponavlja se ovde i tekstom)
        }
      }
    }
  ...
  function nacrtajOblasti(cont, opts) {
    ...
    const stat = (qq) => tacnostSpiska(qq.map((q) => q.id));
    const MALO = 5;   // ispod ovoliko odgovora procenat je slučajnost, ne mera
    const accHtml = (st) => !opts.tacnost ? '' : `<span class="catAcc">${st.acc === null ? '—' : st.seen < MALO ? `<span class="mut">—<span class="samoZaCitace"> ${escapeHtml(L('maloPodataka'))}</span></span>` : `<span class="${accClass(st.acc)}">${st.acc}%</span>`}</span>`;
  ...
  function renderHome() {
    ...
    if (S.lastSec) {
      ...
      if (p > 0 && p < si.ids.length) {
        ...
        const lastKlasa = (S.plan && planIds().length) ? 'secondary' : 'primary';
    ...
    // uz dnevni plan „danas" nije tačno: plan bira koja od onih na redu uzima danas (po vrednosti)
    const uzPlan = (() => {
      if (!(S.plan && S.plan.auto) || !(danaDoIspita() >= 0)) return null;
      const r = planAuto(); if (!r) return null;
      const naRedu = new Set(ready);
      const uzima = r.uListi && Number.isFinite(r.uListi.naRedu) ? r.uListi.naRedu : r.ids.filter((id) => naRedu.has(id)).length;
      // lista za danas urađena (ili bez novih): rečenica o „prednosti novim" bi bila netačna (revizija v150, S2)
      if (!uzima && !r.ids.some((id) => !(S.q[id] && S.q[id].a > 0))) return null;
      return { uzima };
    })();
    ...
    const redovi = [
      ...
      (uRedu || osv) ? brRed(L('brZaPon'), `<b>${uRedu}</b>${uRedu ? ` = ${ready.length} ${L(S.plan && S.plan.auto ? 'ponNaRedu' : 'ponDanas')} + ${waiting.length} ${L('ponKasnije')}` : ''}${osv ? ` · <b>${osv}</b> ${L('osveziLbl')}` : ''}${uzPlan && ready.length ? ` · ${escapeHtml(uzPlan.uzima ? L('planUzima').split('@1').join(uzPlan.uzima) : L('planUzimaNula'))}` : ''}`) : '',
      danasN ? brRed(L('brDanasOdg'), `${nQ(danasN)}, ${S.day.ok} ${L('okShort')}`) : '',
      mk ? brRed(L('brObelezeno'), `<b>${mk}</b>`) : '',
    ].filter(Boolean).join('');
    const planGore = !!(S.plan && S.plan.auto);
    el('homeSummary').innerHTML = (planGore ? planBlok() : '') + redovi
      ...
      + lastChip + homeExtras() + (planGore ? '' : planBlok());
    ...
    }
    {
      ...
      if (bpv) bpv.addEventListener('click', () => {
        ...
        startList(ids, () => L(S.plan && S.plan.auto ? 'planNaslovAuto' : 'planNaslov'), () => L('planNemaSta'), 'filter', { origin: () => renderHome(), plan: true });
      });
      const bpj = el('btnPlanJos');
      if (bpj) bpj.addEventListener('click', planJos);
      const bsa = el('btnStoAko');
      if (bsa) bsa.addEventListener('click', () => {
        const r2 = planAuto(0, true);
        const t = el('stoAkoTelo');
        if (t) t.innerHTML = stoAkoHtml(r2 && r2.stoAko);
        bsa.remove();
      });
      const bpd = el('btnPlanDetalji');
      if (bpd) {
        sklopivo(bpd, null, el('planDetaljiTelo'));
        bpd.addEventListener('click', () => { planDetaljiOtvoreno = bpd.getAttribute('aria-expanded') === 'true'; });
      }
      const bpu = el('btnPlanUkljuci');
      if (bpu) bpu.addEventListener('click', () => {
        const inp = el('planPozivDatum');
        const v = inp ? inp.value : '';
        ocistiPoruku(inp);
        if (!v) { poruciUzPolje(inp, L('planPozivBezDatuma')); return; }
        const greska = greskaDatumaIspita(v);
        if (greska) { poruciUzPolje(inp, greska); return; }
        S.examDate = v;
        if (!S.plan) S.plan = { auto: 1 };
        ponistiAutoKvotu();
        save(); renderHome();
        poruci(L('planPozivUkljucen'));
      });
      const bpne = el('btnPlanNe');
      if (bpne) bpne.addEventListener('click', () => { S.planNe = 1; save(); renderHome(); poruci(L('planPozivSkriven')); });
    }
    const bls = el('btnLastSec');
    if (bls) bls.addEventListener('click', () => {
      const key = S.lastSec;
      const si = secInfo(key);
      startList(si.ids, secTitleFn(key), null, 'section', { secKey: key, startAt: S.secPos[key] || 0, origin: () => browse(key), hash: '#/vezba/' + key });
    });
    ...
    {
      const osv = zaOsvezavanje().length;
      el('mDrillSub').textContent = `${ready.length} ${L(S.plan && S.plan.auto ? 'ponNaRedu' : 'ponDanas')} · ${waiting.length} ${L('ponKasnije')}${osv ? ` · ${osv} ${L('osveziLbl')}` : ''}`;
    }
    el('mMarked').textContent = L('marked');
    el('mMarkedSub').textContent = nQ(mk);
    el('mSim').textContent = L('sim');
    {
      ...
      el('mSimSub').textContent = L('simSub') + (pct === null ? '' : ` · ${L('sansaDanasKratko')} ${pct}%`);
    }
    ...
    if (!S.sims.length) sh.innerHTML = `<h3>${L('history')}</h3><p class="mut">${L('noSims')}</p>`;
      ...
      const grafSim = simSvg();
      const ucinak = !prave.length ? (S.sims.length ? `<p class="mut napomena">${L('simNijednaPrava')}</p>` : '') : `<p class="mut napomena">${L('simUcinak').split('@1').join(polozeno).split('@2').join(prave.length).split('@3').join(poeni(prosek))}${van ? L('simUcinakVan').split('@1').join(van) : ''}</p>`;
      sh.innerHTML = `<h3>${L('history')}</h3>${ucinak}${grafSim}<p class="mut napomena">${L('historyTip')}</p>` + redovi.slice(0, NOVIJIH).join('')
    ...
    if (!cardKeys.length) pk.style.display = 'none';
      ...
      const GRUPE = [
        ...
        ['grp2', ['prvenstvo-prolaza', 'policajac-znaci', 'znakovi-porodice', 'znakovi-opasnosti', 'znakovi-naredbi', 'znakovi-obavestenja', 'zn-ob-kraj-zone', 'zn-ob-parovi', 'zn-ob-autoput', 'zn-ob-vodjenje', 'zn-ob-ostalo', 'semafori', 'oznake-kolovoz', 'svetlosne-oznake']],
        ['grp3', ['kretanje-po-putu', 'moped-motocikl-voznja', 'skretanje', 'preticanje', 'parkiranje', 'parking-table', 'pokazivaci', 'svetla']],
      ...
      // koliko pitanja kojima je ova kartica objašnjenje u proseku dolazi na jednom ispitu (verovatnoća
      // pojavljivanja pitanja iz šablona, po pitanju — ne po podoblasti, jer se deo pitanja vezuje drugde)
      const naIspK = (() => {
        const pp = verovatnocaPojave(), m = {};
        for (const q of Q) {
          const e = EX.byQ[q.id] || {};
          if (e.nocard) continue;
          const k = e.card || EX.bySub[q.sub];
          if (k) m[k] = (m[k] || 0) + (pp.get(q.id) || 0);
        }
        return m;
      })();
      const naIspTxt = (k) => { const v = naIspK[k] || 0; return v < 0.05 ? L('pojRetko') : L('pojNaIspitu').split('@1').join(v.toFixed(1).replace('.', ',')); };
      const entry = (k) => `<div class="pojEntry"><button class="explCardBtn pojBtn" data-poj="${escapeHtml(k)}">📖 ${escapeHtml(T(EX.cards[k].t))} <span class="mut pojIsp">· ${escapeHtml(naIspTxt(k))}</span></button><div class="explCard" style="display:none"></div></div>`;
    ...
    // način računanja cilja: po tempu (plan.js), moji brojevi, ili bez cilja
    const nacin = S.plan && S.plan.auto ? 'auto' : (S.plan || rucnoUI) ? 'rucno' : 'nista';
    el('dataTools').innerHTML = `<button type="button" class="explCardBtn pojBtn" id="btnPodesavanja">${L('podesavanjaDugme')}</button>
      ...
      <div class="podGrupa">
        ...
        <div class="mut napomena">${persistNoteTekst()}</div>
        <div class="podDugmad">
      ...
      <div class="podGrupa" id="planGrupa">
        <h4 class="grupaNaslov">${L('planNaslovAuto')}</h4>
        <div class="tekstSm"><label>${L('examDateLabel')}
          <input type="date" id="examDate" value="${S.examDate || ''}"></label></div>
        ${S.plan && S.plan.auto && !S.examDate ? `<div class="unosPoruka">${L('planAutoBezDatuma')}</div>` : ''}
        <div class="mut razmakG">${L('planNacinPitanje')}</div>
        <div class="podDugmad segment" role="group" aria-label="${escapeHtml(L('planNacinPitanje'))}">
          <button type="button" class="secondary prekidac" id="btnPlanAuto" aria-pressed="${nacin === 'auto' ? 'true' : 'false'}">${L('planNacinAuto')}</button>
          <button type="button" class="secondary prekidac" id="btnPlanRucno" aria-pressed="${nacin === 'rucno' ? 'true' : 'false'}">${L('planNacinRucno')}</button>
        </div>
        <div id="planBlokAuto"${nacin === 'auto' ? '' : ' style="display:none"'}>
          <div class="mut napomena">${L('autoOpis')}</div>
          <div class="podDugmad"><button type="button" class="secondary prekidac" id="btnPlanPod" aria-pressed="${S.plan && S.plan.pod ? 'true' : 'false'}">${L('podNaslov')}</button></div>
          <div class="mut napomena">${L('podOpis')}</div>
        </div>
        <div id="planBlokRucno"${nacin === 'rucno' ? '' : ' style="display:none"'}>
          ...
          <div class="podDugmad"><button type="button" class="secondary prekidac" id="btnPlanPrio" aria-pressed="${S.plan && S.plan.prio ? 'true' : 'false'}">${L('prioNaslov')}</button></div>
          <div class="mut napomena">${L('prioOpis')}</div>
        </div>
        ${nacin === 'nista' ? `<div class="mut napomena">${L('planNacinNista')}</div>` : ''}
        <div class="planPolja"${nacin === 'rucno' || (nacin === 'auto' && S.plan.pod) ? '' : ' style="display:none"'}>
        ...
        ${autoUkljucen ? `<div class="mut napomena" style="display:none">${L('autoPoljaZakljucana')}</div>` : ''}${autoSaPodom ? `<div class="mut napomena">${L('autoPoljaPod')}</div>` : ''}
        <div class="podDugmad">
          <button type="button" class="secondary" id="btnPlanSave"${autoUkljucen ? ' disabled' : ''}${nacin === 'rucno' || autoSaPodom ? '' : ' style="display:none"'}>${L('planSacuvaj')}</button>
          <button type="button" class="secondary" id="btnPlanPredlog"${autoUkljucen ? ' disabled' : ''}${nacin === 'rucno' ? '' : ' style="display:none"'}>${L('planPredlozi')}</button>
    ...
    });
    {
      ...
      const kaziPosle = (t) => {
        ...
        const dn = el('donjaNav');
        const dno = (window.innerHeight || 0) - (dn && getComputedStyle(dn).display !== 'none' ? dn.offsetHeight : 0);
        // tačan pomeraj (scrollIntoView + scroll-margin je u nekim pregledačima ostavljao poruku ispod trake)
        if (r.bottom > dno) window.scrollBy({ top: r.bottom - dno + 12, left: 0, behavior: 'instant' });
        else if (r.top < 0) window.scrollBy({ top: r.top - 12, left: 0, behavior: 'instant' });
      };
      ...
      el('btnPlanAuto').addEventListener('click', () => {
        if (S.plan && S.plan.auto) return;   // već izabrano — izbor se menja drugim dugmetom
        S.plan = { ...(S.plan || {}), auto: 1 };
        rucnoUI = false;
        ponistiAutoKvotu();
        save(); osveziPodesavanja();
        poruci(!S.examDate ? L('autoUkljucenBezDatuma') : S.plan.pod ? L('autoUkljucenPod') : L('autoUkljucen'));
      });
      el('btnPlanRucno').addEventListener('click', () => {
        if (S.plan && !S.plan.auto) return;
        const imaBrojeve = !!(S.plan && (S.plan.novih || S.plan.pon));
        S.plan = S.plan ? { ...S.plan, auto: 0 } : null;
        if (S.plan && planPrazan(S.plan)) S.plan = null;
        rucnoUI = true;   // polja moraju da se vide i pre prvog upisanog broja
        ponistiAutoKvotu();
        save(); osveziPodesavanja();
        poruci(imaBrojeve ? L('autoIskljucen') : L('rucnoUpisi'));
        const pn2 = el('planNovih'); if (pn2 && !pn2.disabled) pn2.focus({ preventScroll: true });
      });
      ...
      el('btnPlanPredlog').addEventListener('click', () => {
        ...
        const dana = danaDoIspita();
        if (dana === 0) { poruciUzPolje(el('examDate'), L('planIspitDanas')); return; }   // datum NIJE prošao — ispit je danas
  ...
  document.addEventListener('click', (ev) => {
    ...
    if (crtez) {
      ...
      const slika = meta.tagName === 'IMG' ? meta : meta.querySelector('img.qImg, img.blizSlika');
      if (!slika) return;
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'ArrowRight') {
      ...
      // Enter mora da uradi ono što je POD FOKUSOM. Ranije je uvek klikao glavno dugme i
      // gasio podrazumevanu radnju, pa je Enter na „Kraj ispita", „Izveštaj", „‹ Prethodno"
      // ili na slici davao SLEDEĆE PITANJE — radnju koju čovek nije tražio, a na ispitu je
      // nepovratna. Isto pravilo već postoji kod vodiča (vidi prečice za turu).
      const f = document.activeElement;
      const naIzboru = !!(f && f.classList && f.classList.contains('choice'));
      // Jedini izuzetak: ponuđen odgovor u UČENJU. Tamo odgovor tek treba potvrditi, pa
      // Enter znači „Potvrdi" — tako i piše u podsetniku za tastaturu. U ISPITU potvrde
      // nema (izbor se beleži odmah), pa tamo Enter samo prebacuje izbor, kao i razmak.
      const pustiPregledacu = f && f.closest && f.closest('button, a[href], summary, svg[data-zum]') && !(naIzboru && qv);
      if (!pustiPregledacu) {
  ...
  try { window.matchMedia('(max-width: 420px)').addEventListener('change', () => applyFont()); } catch (ignore) { /* stari pregledač */ }
  function applyFont() {
    ...
    // uzak ekran + krupna slova: naziv u traci se sklanja (ostaje ikonica), da dugme „nazad" ne bude
    // presečeno u „‹ Спи…" (revizija v149, 125% u ćirilici na 375 px)
    document.body.classList.toggle('uskoKrupno', f >= 1.2 && window.matchMedia('(max-width: 420px)').matches);
  ...
  function renderInstallHint() {
    ...
    strip.id = 'iosHint';
    // ko već ima napredak, mora da zna da ga ikona NE nosi sa sobom
    const imaNapredak = Object.keys(S.q).length > 0;
    strip.innerHTML = `<span>${L('iosHint')}${imaNapredak ? ' ' + L('iosHintNapredak') : ''}</span><button type="button" class="zatvoriX" id="iosHintX" aria-label="Zatvori">✕</button>`;
