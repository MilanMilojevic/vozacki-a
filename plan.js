(function () {
'use strict';
// DNEVNI PLAN (plan.js) — iz tools/plan/prognoza.mjs, pobednika merenja v149.
// Klasična skripta: učitavaju je i aplikacija (index.html) i merni alat (tools/plan/isporuka.mjs →
// tools/plan-proba.mjs), pa se meri TAČNO ono što se isporučuje. Izlaz: self.VozackiPlan.planDanas(api, ctx).
// Procena (šansa, parametri učenja) dolazi iz procena.js, koji se učitava PRE ovog fajla.
//
// Zašto nov plan: stari je računao „koliko TREBA" (fiksan rok za novo gradivo, pa kvote) i nije znao
// koliko čovek stvarno stiže. Mereno na veštačkim učenicima koji ga prate (tools/plan-proba.mjs):
// tražio je više od stvarnog kapaciteta 61–96% dana (do ×48), uz lažnu utehu do 81% i lažnu uzbunu
// do 76%. Ovaj plan traži koliko stižeš u 4 od 5 dana i to vreme troši tamo gde najviše vredi.
//
// Originalni opis (prognoza.mjs):
...
// Tekst plana nastaje sa brojevima i oblicima uz broj, pa ga STR ne može da drži unapred; ćirilica
// se pravi istim preslovljavanjem kao objašnjenja (tools/build-explanations.mjs: toCyr). U tekstu
// plana nema stranih reči ni oznaka koje moraju da ostanu latinicom.
...
// Slab prior (nezavisna provera v149, D2): jedan loš dan na POČETKU plana nije mera kapaciteta. Bez
// ovoga je jedan dan od 15 pitanja (kod čoveka koji stiže 100) oborio zahtev na 15, a posle toga svaki
// dan stignut „tačno koliko je traženo" samo je potvrđivao 15 — oporavak +5% dnevno.
// Prior = PRIOR_DANA zamišljenih dana „mogu bar P" (P = tvoja navika pre plana ili 60). Slabi sa svakim
// danom plana, pa već posle šest dana ne postoji; dva loša dana i dalje obaraju procenu, kao i treba.
const PRIOR_DANA = 5;
const ODSKOK = 0.6;
// Dan ispita: samo kratko ponavljanje onoga što najviše vredi, bez simulacije (D4).
...
function kapacitet(S, api, danas, nSim) {
  ...
  const obs = dani.map((x) => {
    const z = tr[x.d];
    if (!z || typeof z !== 'object') return { v: x.n, c: true, plan: false };
    // simulacija posle prvog računa plana: oduzima se koliko je ONA dodala dnevnom broju (s.dn) — pitanje
    // već odgovoreno tog dana se ne broji dvaput, pa „41 po simulaciji" je dan urađen do kraja
    // proglašavao nestignutim (D1: 12–19% takvih dana). Stari zapisi bez dn: 41.
    const posleSim = sims.filter((s) => api.localDay(s.d) === x.d && s.d > z.t).reduce((a, s) => a + (Number.isFinite(s.dn) ? s.dn : nSim), 0);
    const odPlana = x.n - z.n0 - posleSim;
  ...
  for (let i = 0; i < tacni.length; i++) {
    ...
    // prior ulazi samo protiv IZRAZITO lošeg dana (ispod 60% navike): umereno sporiji dan je mera odmah
  ...
  let est = planCenz.length ? Math.max(...planCenz) * korak : cenz.length ? kvantil(cenz, 0.5) : P;
  // pod od 60 važi samo dok plan nema nijedan svoj dan: sporom učeniku koji uredno stiže 20 dnevno
  // zahtev je inače skakao na 60 čim iz prozora izađe poslednji nestignut dan (nezavisna provera N1)
  if (!tacni.length) est = planCenz.length ? est : Math.max(PODRAZUMEVANI_KAP, est);
  else est = Math.max(est, tacni[tacni.length - 1]);
  return { est: Math.round(est), izvor: planCenz.length ? (tacni.length ? 'mereno' : 'proba') : cenz.length ? 'slobodno' : 'nema' };
}
...
function planDanas(api, ctx) {
  ...
  // „Moj tempo je najmanje ovo" (podešavanja): ručni broj je DONJA granica kapaciteta
  if (ctx.minKap > kap.est) { kap.est = Math.round(ctx.minKap); kap.izvor = 'podesavanje'; }
  ...
  // na dan ispita nema simulacije ni punog dnevnog obima — samo kratko ponavljanje (D4)
  // simulaciji se čuva mesto samo kad uzima najviše pola dnevnog obima; inače bi pojela ceo dan, lista bi
  // ostala prazna, a dan bez odgovora se ne računa ni u tempo — pa bi tako ostalo danima (N2)
  const rezerva = radiSim && !simDanas && !danIspita && kap.est >= 2 * K.nSim ? K.nSim : 0;
  const trazi = Math.max(0, Math.round(Math.min(kap.est - (dan.n || 0) - rezerva, danIspita ? ISPIT_DAN_NAJVISE : Infinity)));
  ...
  const osnovni = kand.slice(0, trazi);
  // ctx.jos: posle ispunjenog plana — sledeća pitanja iz ISTOG poretka (dodatak, ne deo plana)
  const izbor = ctx.jos > 0 ? kand.slice(0, trazi + ctx.jos) : osnovni;
  const ids = izbor.map((k) => k.id);
  const nPon = osnovni.filter((k) => k.vrsta === 'p').length, nNov = osnovni.length - nPon;
  ...
  if (!pg.tr[danas]) {
    // k = koliko stavki liste stvarno podiže dnevni broj: pitanje već odgovoreno danas (npr. pogrešno u
    // jutarnjoj simulaciji) ostaje u listi — ponovo ga vežbaš — ali se u dnevni broj ne računa dvaput,
    // pa bez ovoga urađen dan izgleda nestignut (nezavisna provera D1)
    const vecDanas = (id) => { const r = S.q[id]; return !!(r && r.last && api.localDay(r.last) === danas); };
    pg.tr[danas] = { n0: dan.n || 0, k: osnovni.filter((x) => !vecDanas(x.id)).length, t: sad };
  ...
  const red = [];
  if (danIspita) red.push(`Ispit je danas: ako vežbaš pre polaganja, samo kratko — najviše ${ISPIT_DAN_NAJVISE} pitanja koja na ispitu najviše vrede, bez simulacije.`);
  const glava = `procena za dan ispita je ≈ ${E} od ${K.ukupno} poena (prag ${prag}), šansa za prolaz oko ${pr(b.P)}`;
  const pokr = pro && Number.isFinite(pro.pokriveno) ? Math.round(100 * pro.pokriveno) : null;
  red.push(ocena === 'nema' ? `Šansu još ne računam: vežbaj pitanja iz bar trećine ispita${pokr === null ? '' : ` (sada je pokriveno ${pokr}%)`} ili uradi jednu simulaciju i odgovori na sva pitanja — broj bez dokaza bi bio nagađanje.` : ocena === 'ok' ? `✅ Stižeš: ovim tempom ${glava}.` : ocena === 'upozorenje' ? `⚠ Na ivici: ovim tempom ${glava}.` : `⛔ Ovim tempom ne stižeš do praga: ${glava}.`);
  red.push(!kand.length
    ? 'Danas nema ničega na redu: sva pitanja su viđena, a nijedno ponavljanje nije na redu.'
    : osnovni.length
    ? `Danas: ${br(osnovni.length, 'pitanje', 'pitanja', 'pitanja')} (${br(nPon, 'ponavljanje', 'ponavljanja', 'ponavljanja')}, ${br(nNov, 'novo', 'nova', 'novih')}), poređana po tome koliko vrede na ispitu${danIspita ? '.' : ' — ako stigneš više, posle njih nastavi istim redom.'}` + (rezerva ? ` Uz to simulacija, kao i obično.` : simDanas ? ' Simulacija za danas je urađena.' : '')
    : rezerva ? 'Lista za danas je urađena — ostaje još simulacija.'
    : 'Danas si već uradio koliko obično stižeš — sve preko toga je dodatak.');
  // simulacija ponekad (ređe od svakog drugog dana): plan joj ne čuva mesto, pa se kaže kako da ne
  // padne POVRH liste (nezavisna provera D3) — posle simulacije se lista sama skrati
  if (!danIspita && !rezerva && uc.sim > 0 && !simDanas && osnovni.length) red.push('Ako danas radiš i simulaciju, uradi je pre liste — ona troši isti dnevni obim, pa se lista sama skrati.');
  const simTxt = radiSim ? ' zajedno sa simulacijom' : '';
  const aktTxt = uc.akt < 0.85 ? `; radiš otprilike ${Math.round(uc.akt * 7)} od 7 dana` : '';
  if (!danIspita) red.push(kap.izvor === 'podesavanje'
    ? `Plan računa sa ~${kap.est} pitanja dnevno${simTxt} — tvoj najmanji tempo iz podešavanja${aktTxt}.`
    : kap.izvor === 'nema'
    ? `Tvoj tempo još ne znam, pa plan polazi od ~${kap.est} pitanja dnevno; posle par dana ravna se po tebi.`
    : kap.izvor === 'slobodno'
  ...
  const nev = Math.round(baza.nevidjeno), nula = Math.round(baza.nulaNevid), nevUk = nev + nula, novihDo = Math.round(baza.novih), ponDo = Math.round(baza.pon);
  const dTxt = `do ispita (${br(D, 'dan', 'dana', 'dana')})`;
  if (dIsp > 0) red.push(nevUk === 0
    ? `Ovim tempom ${dTxt} vidiš sva pitanja i uradiš ~${br(ponDo, 'ponavljanje', 'ponavljanja', 'ponavljanja')}.`
    : `Ovim tempom ${dTxt}: još ~${br(novihDo, 'novo pitanje', 'nova pitanja', 'novih pitanja')}${ponDo ? ` i ~${br(ponDo, 'ponavljanje', 'ponavljanja', 'ponavljanja')}` : ''}; ostaje ~${br(nevUk, 'neviđeno', 'neviđena', 'neviđenih')}${nula > 0 && nev > 0 ? ` — od toga ${nula} van šablona ispita (po dosadašnjim proverama se ne izvlače), a ~${nev} se na ispitu retko pojavljuju` : nula > 0 ? ' — sva van šablona ispita (po dosadašnjim proverama se ne izvlače)' : ' — ona koja se na ispitu najređe pojavljuju'}.`);
  ...
  if (!danIspita && (ocena === 'upozorenje' || ocena === 'problem')) {
  ...
  return { ids, ocena, tekst: red.join(String.fromCharCode(10)), redovi: red.map((l) => ({ l, c: uCir(l) })), projekcija: proj, kap: kap.est, trazi, rezerva, simDanas, nemaNaRedu: !kand.length,
    // cilj dnevne trake u aplikaciji: na dan ispita samo kratko ponavljanje, a kad ništa nije na redu — nema cilja
    cilj: !kand.length ? null : danIspita ? (dan.n || 0) + trazi : kap.est };
}
